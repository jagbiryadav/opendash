...
const model = "opendash/gpt-5.3-codex"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with opendash...`)
  ...
  try {
    await $`opendash run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  opendash failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with opendash")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with opendash...")
