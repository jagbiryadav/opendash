...
<p align="center">
  <img src="assets/readme/opendash-banner.png" alt="OpenDash" width="700">
</p>
...
<p align="center">
  <a href="https://opendashai.netlify.app/zh/opendash">官网</a> | <a href="https://opendashai.netlify.app/zh/blog/opendash">博客</a>
</p>

---

OpenDash 是一个终端原生的 AI 编程助手。它能读写代码、执行命令、管理 Git，通过持久化记忆系统，在多次会话间保持对你项目的深度理解，并自我进化。

内置 OpenDash 限时免费通道——零配置即可开始使用。也支持接入各家主流 LLM 厂商 API。
...
中日韩字符可能显示为乱码。OpenDash 已为 PowerShell/cmd 子进程强制开启 UTF-8 输出。
...
除了 OpenDash，小米 MiMo 模型也能在 Cursor、Cline、Zed 等各种 Agent 和编程工具里使用。
...
通过项目目录下的 `.opendash/opendash.json`（或全局 `~/.config/opendash/opendash.json`）配置。主要选项包括：
...
OpenDash 基于 [OpenDash](https://github.com/opendash/opendash) fork 构建，保留其全部核心能力（多 Provider、TUI、LSP、MCP、插件），并在此基础上构建了持久化记忆、智能上下文管理、子智能体编排、目标驱动的自主循环、Compose 工作流，以及通过 dream/distill 实现的自我进化。
...
使用 OpenDash 还需遵守[使用限制](./USE_RESTRICTIONS.md)。
使用小米 MiMo 托管服务须遵守 [MiMo 服务条款](https://platform.opendashai.netlify.app/docs/terms/user-agreement)。
