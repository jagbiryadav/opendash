...
You can add external tools to OpenDash using the _Model Context Protocol_, or MCP. OpenDash supports both local and remote servers.
...
You can define MCP servers in your [OpenDash Config](https://opendashai.netlify.app/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/opendash` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenDash automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenDash will:
...
If the server requires authentication, OpenDash will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opendash mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenDash will store the tokens securely in `~/.local/share/opendash/mcp-auth.json`.
...
Your MCPs are available as tools in OpenDash, alongside built-in tools. So you can manage them through the OpenDash config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenDash to your Sentry account.
