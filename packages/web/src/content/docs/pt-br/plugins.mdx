---
title: Plugins
description: Escreva seus próprios plugins para estender o opendash.
---

Plugins permitem que você estenda o opendash conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do opendash.
...
- `.opendash/plugins/` - Plugins em nível de projeto
- `~/.config/opendash/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/opendash/node_modules/`.
...
1. Configuração global (`~/.config/opendash/opendash.json`)
2. Configuração do projeto (`opendash.json`)
3. Diretório de plugins global (`~/.config/opendash/plugins/`)
4. Diretório de plugins do projeto (`.opendash/plugins/`)
...
O opendash executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do opendash para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o opendash.
...
:::note
Se você estiver usando o aplicativo desktop opendash, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o opendash de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao opendash:
...
O helper `tool` cria uma ferramenta personalizada que o opendash pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o opendash junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://opendashai.netlify.app/docs/sdk) para detalhes.
