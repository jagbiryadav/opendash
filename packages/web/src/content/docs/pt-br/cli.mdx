---
title: CLI
description: Opções e comandos da CLI do opendash.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do opendash, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
opendash
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o opendash programaticamente.
...
Inicie a interface de usuário do terminal do opendash.
...
A CLI do opendash também possui os seguintes comandos.

---

### agent

Gerencie agentes para o opendash.
...
Anexe um terminal a um servidor backend do opendash já em execução, iniciado via comandos `serve` ou `web`.

```bash
opendash attach [url]
```

Isso permite usar o TUI com um backend opendash remoto. Por exemplo:
...
O opendash é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `opendash auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opendash/auth.json`.

```bash
opendash auth login
```

Quando o opendash é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no opendash.
...
Execute o opendash em modo não interativo passando um prompt diretamente.
...
```bash "opendash run"
opendash run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `opendash serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
opendash run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| `--attach`   |       | Anexe a um servidor opendash em execução (por exemplo, http://localhost:4096) |
...
Inicie um servidor opendash sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
opendash serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opendash sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opendash`).
...
Liste todas as sessões do opendash.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do opendash.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do opendash.

```bash
opendash import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do opendash.
...
Inicie um servidor opendash sem cabeça com uma interface web.

```bash
opendash web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o opendash através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opendash`).
...
Desinstale o opendash e remova todos os arquivos relacionados.
...
Atualiza o opendash para a versão mais recente ou uma versão específica.
...
A CLI do opendash aceita as seguintes flags globais.
...
O opendash pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opendash`) |
