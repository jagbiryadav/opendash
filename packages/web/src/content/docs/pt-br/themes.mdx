...
Com o opendash, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o opendash usa nosso próprio tema `opendash`.
...
O opendash vem com vários temas embutidos.
...
- Querem que o opendash corresponda à aparência do seu terminal
...
O opendash suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/opendash/themes/*.json` ou `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.opendash/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.opendash/themes/*.json`
