...
As habilidades do agente permitem que o opendash descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O opendash pesquisa nesses locais:

- Configuração do projeto: `.opendash/skills/<name>/SKILL.md`
- Configuração global: `~/.config/opendash/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o opendash sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.opendash/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.opendash/skills/git-release/SKILL.md` assim:
...
O opendash lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `opendash.json`:
...
**Para agentes embutidos** (em `opendash.json`):
