...
:::caution
Modes are now configured through the `agent` option in the opendash config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in opendash allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the opendash config.
...
opendash comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.opendash/plans/*.md` to detail the plan itself
...
Configure modes in your `opendash.json` config file:
...
both the global opendash config and the project specific config.
...
Create mode files in `.opendash/modes/` for project-specific modes or `~/.config/opendash/modes/` for global modes:
