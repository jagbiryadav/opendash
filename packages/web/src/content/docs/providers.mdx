---
title: Providers
description: Using any LLM provider in OpenDash.
---

import config from "../../../config.mjs"
export const console = config.console

OpenDash uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenDash config.
...
You can customize the providers through the `provider` section in your OpenDash
config.
...
OpenDash Zen is a list of models provided by the OpenDash team that have been
tested and verified to work well with OpenDash. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenDash Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenDash Zen`, and head to [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
It works like any other provider in OpenDash and is completely optional to use.

---

## OpenDash Go

OpenDash Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the OpenDash team that have been
tested and verified to work well with OpenDash.

1. Run the `/connect` command in the TUI, select `OpenDash Go`, and head to [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
To use Amazon Bedrock with OpenDash:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running opendash:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `opendash.json`:
...
OpenDash. Anthropic explicitly prohibits this.

Previous versions of OpenDash came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in OpenDash with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for opendash to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your opendash config.
...
GitLab Duo support in OpenDash is experimental. Features, configuration, and
behavior may change in future releases.
:::

OpenDash integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
OpenDash uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenDash
to only use your own GitLab-hosted instance, add the following to your
`opendash.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, OpenDash will:
...
4. Route tool execution requests through OpenDash's permission-gated tool system
...
To use your GitHub Copilot subscription with opendash:
...
To use Google Vertex AI with OpenDash:
...
2. Set the required environment variables:
   ...
   Set them while running opendash.
...
In the event you see a feature or model from Helicone that isn't configured automatically through opendash, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session) plugin to automatically log each OpenDash conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenDash conversation listed as a separate session.
...
You can configure opendash to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure opendash to use local models through LM Studio.
...
You can configure opendash to use local models through Ollama.

:::tip
Ollama can automatically configure itself for OpenDash. See the [Ollama integration docs](https://docs.ollama.com/integrations/opendash) for details.
:::
...
To use Ollama Cloud with OpenDash:
...
3. Copy the API key for use in OpenDash.
...
6. **Important**: Before using cloud models in OpenDash, you must pull the model information locally:
...
OpenDash Zen is a list of tested and verified models provided by the OpenDash team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenDash Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenDash Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenDash API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your opendash config.
...
5. You can also customize them through your opendash config. Here's an example of specifying a provider
...
You can also customize models through your opendash config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with opendash. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `opendash.json` file in your project directory:
...
The `limit` fields allow OpenDash to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `opendash auth list` to see if the credentials
...
2. For custom providers, check the opendash config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your opendash config.
