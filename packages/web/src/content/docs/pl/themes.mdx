...
Dzięki opendash możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie opendash używa naszego własnego motywu `opendash`.
...
opendash ma kilka wbudowanych motywów.
...
- Chcą, aby OpenDash pasował do wyglądu ich terminala
...
opendash obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/opendash/themes/*.json` lub `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.opendash/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.opendash/themes/*.json`
