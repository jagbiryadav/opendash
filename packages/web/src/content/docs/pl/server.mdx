---
title: Serwer
description: Interakcja z serwerem opendash za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `opendash serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient opendash.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `opendash.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `opendash` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `opendash serve`, jak i `opendash web`.
...
Kiedy uruchomisz `opendash`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera opendash do programowej interakcji z opendash.
:::

Ta architektura umożliwia obsługę wielu klientów przez opendash i programową interakcję z kodem otwartym.

Możesz uruchomić `opendash serve`, aby uruchomić samodzielny serwer. Jeśli masz
opendash TUI działa, `opendash serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki opendash [IDE](/docs/ide).
...
Serwer opendash udostępnia następujące interfejsy API.
