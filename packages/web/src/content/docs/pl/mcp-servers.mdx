...
Możesz dodać dodatkowe informacje do opendash za pomocą _Model Context Protocol_, w skrócie MCP. opendash obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [opendash Config](https://opendashai.netlify.app/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/opendash`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
opendash automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, opendash:
...
Jeśli serwer wymaga uwierzytelnienia, opendash poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `opendash mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji opendash bezpieczne przechowa tokeny w `~/.local/share/opendash/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w opendash, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie opendash, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć opendash z kontem Sentry.
