---
title: CLI
description: Opcje i polecenia wiersza poleceń OpenDash.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI OpenDash domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
opendash
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać OpenDash programowo.
...
Uruchamia interfejs terminalowy (TUI) OpenDash.
...
Interfejs CLI OpenDash zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami OpenDash.
...
Dołącz terminal do już działającego serwera backendu OpenDash uruchomionego za pomocą `serve` lub `web`.

```bash
opendash attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem OpenDash. Na przykład:
...
OpenDash integruje się z wieloma dostawcami. Możesz użyć `opendash auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opendash/auth.json`.

```bash
opendash auth login
```

Podczas uruchamiania OpenDash ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w OpenDash.
...
Uruchom OpenDash w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `opendash serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| `--attach`   |       | Dołącz do działającego serwera OpenDash (np. http://localhost:4096)     |
...
Uruchom serwer OpenDash (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
opendash serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API OpenDash bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opendash`).
...
Lista wszystkich sesji OpenDash.
...
Pokaż statystyki wykorzystania tokenów i sesji OpenDash.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania OpenDash.
...
Uruchom serwer OpenDash z interfejsem internetowym.

```bash
opendash web
```

Uruchamia to serwer HTTP i udostępnia OpenDash przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opendash`).
...
Odinstaluj OpenDash i usuń wszystkie powiązane pliki.
...
Aktualizuj OpenDash do najnowszej lub określonej wersji.
...
Interfejs CLI OpenDash przyjmuje następujące flagi globalne dla każdego polecenia.
...
OpenDash można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `opendash`)    |
