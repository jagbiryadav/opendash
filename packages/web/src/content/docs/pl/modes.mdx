...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji opendash.
...
te lub skonfiguruj własne za pomocą konfiguracji opendash.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.opendash/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `opendash.json`:
...
- Globalnie: `~/.config/opendash/modes/`
...
Jeśli nie ma wpływu na temperaturę, opendash stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna opendash, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.opendash/modes/` dla trybów zapisanych dla projektu lub `~/.config/opendash/modes/` dla trybów globalnych:
