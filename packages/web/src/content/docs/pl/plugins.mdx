---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć opendash.
---

Wtyczki umożliwiające wyłączenie opendash poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania opendash.
...
- `.opendash/plugins/` - Wtyczki na poziomie projektu
- `~/.config/opendash/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/opendash/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/opendash/opendash.json`)
2. Konfiguracja projektu (`opendash.json`)
3. Globalny katalog wtyczek (`~/.config/opendash/plugins/`)
4. Katalog wtyczek projektu (`.opendash/plugins/`)
...
opendash uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z opendash do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do opendash.
...
:::note
Jeśli korzystasz z aplikacji komputerowej opendash, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj opendash czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do opendash:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod opendash. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://opendashai.netlify.app/docs/sdk).
