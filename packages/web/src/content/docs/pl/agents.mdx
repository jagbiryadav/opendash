...
W OpenDash obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
OpenDash zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
OpenDash ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

OpenDash ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `opendash.json`:
...
- Globalnie: `~/.config/opendash/agents/`
- W projekcie: `.opendash/agents/`
...
Jeśli nie zostanie skonfigurowana, OpenDash używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji OpenDash, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji OpenDash w `provider/model-id`. Na przykład, [OpenDash Zen](/docs/zen), użyj `opendash/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `opendash models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/opendash/opendash).
:::
