...
Aby debugować problemy z opendash, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\opendash\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `opendash --log-level DEBUG`.

---

## Przechowywanie danych

opendash przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/opendash/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\opendash`
...
opendash Desktop uruchamia w tle lokalny serwer opendash (sidecar `opendash-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OpenDash` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opendash/opendash.jsonc` (lub `~/.config/opendash/opendash.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opendash/opendash.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opendash\opendash.jsonc`
...
opendash może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/opendash/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opendash\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby opendash mógł ją odbudować.

1. Całkowicie zamknij opendash Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/opendash`
- **Linux**: usuń `~/.cache/opendash` (lub uruchom `rm -rf ~/.cache/opendash`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\opendash`

3. Uruchom ponownie opendash Desktop.

---

### Napraw problemy z połączeniem z serwerem

opendash Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `opendash.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows opendash Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami opendash.

---

### Brak powiadomień

opendash Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla opendash w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie opendash Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji opendash Desktop):

- `opendash.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `opendash.global.dat` i `opendash.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z opendash:
...
3. Upewnij się, że masz najnowszą wersję z `opendash upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `opendash models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\opendash`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. opendash dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\opendash`

2. Uruchom ponownie kod opendash, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to opendash do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
opendash wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
