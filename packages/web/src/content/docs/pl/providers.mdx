---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w opendash.
---

import config from "../../../../config.mjs"
export const console = config.console

opendash używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji opendash.
...
w `~/.local/share/opendash/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim opendash
config.
...
OpenDash Zen to lista modeli dostarczonych przez zespół opendash, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opendash. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenDash Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenDash Zen` i przejdź do [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Działa jak każdy inny dostawca w opendash i jest całkowicie opcjonalny w użyciu.

---

## OpenDash Go

OpenDash Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opendash, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opendash.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenDash Go` i przejdź do [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Aby używać Amazon Bedrock z opendash:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania opendash:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `opendash.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w opendash nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod opendash działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji opendash.
...
opendash używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować opendash
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`opendash.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `opendash.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem opendash:
...
Aby używać Google Vertex AI z opendash:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania opendash.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą opendash, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session), aby automatycznie rejestrować każdą konwersację opendash jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja opendash będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować opendash tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować opendash tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować opendash tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem opendash. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opendash).
:::
...
Aby korzystać z Ollama Cloud z opendash:
...
3. Skopiuj klucz API do użycia w opendash.
...
6. **Ważne**: Przed użyciem modeli chmurowych w opendash musisz pobrać informacje o modelu lokalnie:
...
OpenDash Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opendash. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenDash Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenDash Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API opendash.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji opendash.
...
5. Możesz je także dostosować za pomocą konfiguracji opendash. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z opendash:
...
Możesz także dostosować modele za pomocą konfiguracji opendash. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z opendash. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `opendash.json` w katalogu projektu:
...
Pola `limit` pozwalają opendash zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `opendash auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację opendash i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji opendash.
