...
Umiejętności agenta pozwalają opendash odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
opendash przeszukuje te lokalizacje:

- Project config: `.opendash/skills/<name>/SKILL.md`
- Global config: `~/.config/opendash/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu opendash przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.opendash/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.opendash/skills/git-release/SKILL.md` w ten sposób:
...
opendash wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `opendash.json`:
...
**Dla agentów wbudowanych** (w `opendash.json`):
