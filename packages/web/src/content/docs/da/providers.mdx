---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenDash.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenDash bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenDash-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/opendash/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenDash-konfigurationen.
...
OpenDash Zen er en liste over modeller leveret af OpenDash-teamet, som er blevet testet og verificeret til at fungere godt med OpenDash. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenDash Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg opendash og gå til [opendashai.netlify.app/auth](https://opendashai.netlify.app/auth).
...
Det fungerer som alle andre udbydere i OpenDash og er helt valgfrit at bruge.

---

## OpenDash Go

OpenDash Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenDash-teamet, som er testet og verificeret til at fungere godt med OpenDash.

1. Kør kommandoen `/connect` i TUI, vælg `OpenDash Go`, og gå til [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Sådan bruger du Amazon Bedrock med OpenDash:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører opendash:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `opendash.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenDash understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at opendash skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem opendash-konfigurationen.
...
gemme tokenet i opendash auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenDash bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenDash
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`opendash.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med OpenDash:
...
Sådan bruger du Google Vertex AI med OpenDash:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører opendash.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem opendash, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session) plugin for automatisk at logge hver OpenDash-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenDash-samtale opført som en separat session.
...
Du kan konfigurere opendash til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere opendash til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere opendash til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenDash. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opendash) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenDash:
...
3. Kopier API-nøglen til brug i OpenDash.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenDash, skal du hente modelinformationen lokalt:
...
OpenDash Zen er en liste over testede og verificerede modeller leveret af OpenDash-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenDash Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenDash Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenDash API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem opendash-konfigurationen.
...
5. Du kan også tilpasse dem gennem opendash-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenDash:
...
Du kan også tilpasse modeller gennem opendash-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med opendash. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `opendash.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenDash forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `opendash auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek opendash-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i opendash-konfigurationen.
