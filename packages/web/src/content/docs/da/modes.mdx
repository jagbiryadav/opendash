...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i opendash-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i opendash giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem opendash-konfigurationen.
...
opendash leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.opendash/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `opendash.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger opendash modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale opendash-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.opendash/modes/` for projektspecifikke tilstande eller `~/.config/opendash/modes/` for globale tilstande:
