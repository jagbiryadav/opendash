...
Med OpenDash kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenDash vårt eget `opendash`-tema.
...
OpenDash kommer med flere innebygde temaer.
...
- Vil at OpenDash skal matche terminalens utseende
...
OpenDash støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/opendash/themes/*.json` eller `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.opendash/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.opendash/themes/*.json`
