...
Der er to typer agenter i OpenDash; primære agenter og subagenter.
...
OpenDash leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
OpenDash leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

OpenDash leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `opendash.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger OpenDash modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale OpenDash-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din OpenDash-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenDash Zen](/docs/zen), vil du bruge `opendash/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `opendash models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/opendash/opendash).
:::
