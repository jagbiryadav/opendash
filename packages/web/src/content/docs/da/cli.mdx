---
title: CLI
description: OpenDash CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenDash CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opendash
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med OpenDash programmatisk.
...
Start OpenDash terminalbrugergrænsefladen.
...
Administrer agenter i OpenDash.
...
Tilslut en terminal til en allerede kørende OpenDash backend-server startet via `serve` eller `web` kommandoer.

```bash
opendash attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern OpenDash backend. For eksempel:
...
OpenDash drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opendash auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opendash/auth.json`.

```bash
opendash auth login
```

Når OpenDash starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i OpenDash.
...
Kør opendash i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `opendash serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| `--attach`   |      | Tilslut til en kørende opendash-server (f.eks. http://localhost:4096)               |
...
Start en hovedløs OpenDash-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
opendash serve
```

Dette starter en HTTP-server, der giver API-adgang til opendash-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opendash`).
...
Administrator OpenDash sessionsholder.
...
Liste over alle OpenDash sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine OpenDash-sessioner.
...
Importer sessionsdata fra en JSON fil eller OpenDash del URL.
...
Start en hovedløs OpenDash-server med en webgrænseflade.

```bash
opendash web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenDash via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opendash`).
...
Afinstaller OpenDash og fjern alle relaterede filer.
...
Opdaterer OpenDash til den nyeste version eller en specifik version.
...
opendash CLI tager følgende globale flag.
...
OpenDash kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opendash`) |
