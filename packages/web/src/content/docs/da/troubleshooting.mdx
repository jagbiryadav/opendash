...
For at fejlfinde problemer med OpenDash, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\opendash\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `opendash --log-level DEBUG`.

---

## Lagring

opendash gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/opendash/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\opendash`
...
OpenDash Desktop kører en lokal OpenDash-server (`opendash-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenDash`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/opendash/opendash.jsonc` (eller `~/.config/opendash/opendash.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/opendash/opendash.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opendash\opendash.jsonc`
...
OpenDash kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opendash/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opendash\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenDash kan genopbygge den.

1. Afslut OpenDash Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/opendash`
- **Linux**: slet `~/.cache/opendash` (eller kør `rm -rf ~/.cache/opendash`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\opendash`

3. Start OpenDash Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenDash Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `opendash.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver OpenDash Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenDash.

---

### Meddelelser vises ikke

OpenDash Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenDash i dine OS-indstillinger, og
...
1. Afslut OpenDash Desktop.
2. Find og slet disse filer (de findes i OpenDash Desktop-appens datamappe):

- `opendash.settings.dat` (skrivebordsstandardserver URL)
- `opendash.global.dat` og `opendash.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OpenDash:
...
For at finde ud af hvilke modeller du har adgang til, kør `opendash models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\opendash`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. opendash installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\opendash`

2. Start opendash på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge opendash til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
opendash vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
