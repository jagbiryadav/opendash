---
title: Server
description: Interager med opendash-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `opendash serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opendash-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `opendash.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `opendash`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `opendash serve` og `opendash web`.
...
Når du kører `opendash` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opendash-serveren til at interagere med opendash programmatisk.
:::

Denne arkitektur lader opendash understøtte flere klienter og lader dig interagere med opendash programmatisk.

Du kan køre `opendash serve` for at starte en selvstændig server. Hvis du har
opendash TUI kørende, vil `opendash serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenDash [IDE](/docs/ide) plugins.
...
OpenDash-serveren viser følgende API'er.
