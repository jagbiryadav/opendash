---
title: ACP Support
description: Use OpenDash in any ACP-compatible editor.
---

OpenDash supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use OpenDash via ACP, configure your editor to run the `opendash acp` command.

The command starts OpenDash as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'OpenDash' agent in the AI Chat agent selector.
...
To use OpenDash as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use OpenDash as the ACP agent for chat.
...
OpenDash works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your OpenDash config
