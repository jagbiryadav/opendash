---
title: CLI
description: OpenDash CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der OpenDash CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
opendash
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit OpenDash interagieren.
...
Starten Sie die Terminalbenutzeroberfläche OpenDash.
...
Der OpenDash CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für OpenDash verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden OpenDash-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
opendash attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend OpenDash. Zum Beispiel:
...
OpenDash wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `opendash auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/opendash/auth.json` gespeichert.

```bash
opendash auth login
```

Wenn OpenDash startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in OpenDash sehen möchten.
...
Führen Sie OpenDash im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `opendash serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| `--attach`   |      | An einen laufenden OpenDash-Server anschließen (z.B. http://localhost:4096)                         |
...
Starten Sie einen Headless-OpenDash-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
opendash serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf OpenDash-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `opendash`).
...
Listen Sie alle OpenDash-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre OpenDash-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer OpenDash-Freigabe-URL.

```bash
opendash import <file>
```

Sie können aus einer lokalen Datei oder einer OpenDash-Freigabe-URL importieren.
...
Starten Sie einen Headless-OpenDash-Server mit einer Weboberfläche.

```bash
opendash web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf OpenDash zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `opendash`).
...
Deinstallieren Sie OpenDash und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert OpenDash auf die neueste Version oder eine bestimmte Version.
...
Der OpenDash CLI akzeptiert die folgenden globalen Flags.
...
OpenDash kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `opendash`) |
