...
Erstellen Sie `.opendash/commands/test.md`:

```md title=".opendash/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die OpenDash-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem OpenDash [config](/docs/config):

```json title="opendash.jsonc" {4-12}
{
...
- Global: `~/.config/opendash/commands/`
- Pro Projekt: `.opendash/commands/`

```markdown title="~/.config/opendash/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".opendash/commands/component.md"
---
...
```md title=".opendash/commands/create-file.md"
---
...
```md title=".opendash/commands/analyze-coverage.md"
---
...
```md title=".opendash/commands/review-changes.md"
---
...
```md title=".opendash/commands/review-component.md"
---
...
OpenDash enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
