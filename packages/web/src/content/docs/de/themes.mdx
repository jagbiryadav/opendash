...
Mit OpenDash kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenDash das `opendash`-Theme.
...
OpenDash liefert mehrere eingebaute Themes mit.
...
- OpenDash optisch ans Terminal anpassen wollen
...
OpenDash unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/opendash/themes/*.json` oder `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.opendash/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.opendash/themes/*.json`
