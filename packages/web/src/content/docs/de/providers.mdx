---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OpenDash.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenDash verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OpenDash-Konfiguration.
...
in `~/.local/share/opendash/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem OpenDash anpassen
config.
...
OpenDash Zen ist eine Liste von Modellen, die vom OpenDash-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OpenDash funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenDash Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in OpenDash und ist völlig optional.

---

## OpenDash Go

OpenDash Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OpenDash-Team getestet und verifiziert wurden, dass sie gut mit OpenDash funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenDash Go` und gehen Sie zu [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
So verwenden Sie Amazon Bedrock mit OpenDash:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OpenDash ausführen:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY opendash

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile opendash

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX opendash
   ```
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `opendash.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OpenDash wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OpenDash ordnungsgemäß funktioniert.
   :::
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX opendash
   ```
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX opendash
   ```
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OpenDash-Konfiguration hinzufügen.
...
um das Token im OpenDash-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OpenDash verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OpenDash
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`opendash.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OpenDash:
...
So verwenden Sie Google Vertex AI mit OpenDash:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OpenDash ausführen.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id opendash
   ```
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OpenDash konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
```jsonc title="~/.config/opendash/opendash.jsonc"
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "provider": {
    "helicone": {
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session)-Plugin, um jede OpenDash-Konversation automatisch als Sitzung in Helicone zu protokollieren.

```bash
npm install -g opendash-helicone-session
```
...
```json title="opendash.json"
{
  "plugin": ["opendash-helicone-session"]
}
```

Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OpenDash-Konversation als separate Sitzung aufgeführt.
...
Sie können OpenDash für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren

```json title="opendash.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "provider": {
    "llama.cpp": {
...
Sie können OpenDash über LM Studio für die Verwendung lokaler Modelle konfigurieren.

```json title="opendash.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "provider": {
    "lmstudio": {
...
Sie können OpenDash über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OpenDash konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opendash).
:::

```json title="opendash.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "provider": {
    "ollama": {
...
So verwenden Sie Ollama Cloud mit OpenDash:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OpenDash.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OpenDash verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenDash Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenDash-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OpenDash Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OpenDash Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OpenDash API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OpenDash-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OpenDash-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
3. Geben Sie Ihren JSON-Service-Account-Key ein.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' opendash
   ```
...
4. Legen Sie optional die Bereitstellung ID und die Ressourcengruppe fest:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group opendash
   ```
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OpenDash:
...
Sie können Modelle auch über Ihre OpenDash-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OpenDash verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
3. Geben Sie Ihren API-Schlüssel für den Anbieter ein.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in opendash.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Erstellen oder aktualisieren Sie Ihre `opendash.json`-Datei in Ihrem Projektverzeichnis:

   ```json title="opendash.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opendashai.netlify.app/config.json",
     "provider": {
       "myprovider": {
...
```json title="opendash.json" {9,11,17-20}
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "provider": {
    "myprovider": {
...
Mithilfe der `limit`-Felder kann OpenDash erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `opendash auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OpenDash-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OpenDash-Konfiguration übereinstimmt.
