...
Wenn OpenDash Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\opendash\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `opendash --log-level DEBUG`.

---

## Speicher

opendash speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/opendash/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\opendash`
...
OpenDash Desktop startet im Hintergrund einen lokalen OpenDash-Server (`opendash-cli`-Sidecar).
...
- Nur macOS: `OpenDash`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/opendash/opendash.jsonc` (oder `~/.config/opendash/opendash.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opendash/opendash.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opendash\opendash.jsonc`
...
OpenDash kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/opendash/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opendash\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OpenDash ihn neu aufbauen kann.

1. Beende OpenDash Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/opendash`
- **Linux**: loesche `~/.cache/opendash` (oder fuehre aus: `rm -rf ~/.cache/opendash`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\opendash`

3. Starte OpenDash Desktop neu.

---

### Server-Verbindungsprobleme beheben

OpenDash Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `opendash.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt OpenDash Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OpenDash-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OpenDash Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OpenDash in den OS-Einstellungen aktiviert sind, und
...
1. Beende OpenDash Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OpenDash Desktop):

- `opendash.settings.dat` (desktop default server URL)
- `opendash.global.dat` und `opendash.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit OpenDash hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `opendash models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\opendash`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. opendash installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\opendash`

2. Starte opendash neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt opendash, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
opendash erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
