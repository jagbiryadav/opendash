---
title: Regeln
description: Lege eigene Anweisungen fuer opendash fest.
---

Du kannst opendash ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in opendash den Befehl `/init` aus.
...
So kann opendash sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht opendash sie zu erweitern.
...
opendash kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/opendash/AGENTS.md` speichern.
Diese gelten in allen opendash-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenDash dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/opendash/AGENTS.md` existiert)
...
Beim Start sucht opendash in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/opendash/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `opendash.json` oder in `~/.config/opendash/opendash.json` angeben.
...
opendash parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit opendash.json

Empfohlen ist das `instructions`-Feld in der `opendash.json`:
...
Du kannst opendash explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `opendash.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
