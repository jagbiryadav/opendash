...
Agent Skills erlauben OpenDash, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenDash sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenDash vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.opendash/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.opendash/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenDash listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `opendash.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `opendash.json`):
