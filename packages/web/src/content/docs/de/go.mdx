...
OpenDash Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
OpenDash Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in OpenDash. Du abonnierst OpenDash Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
OpenDash zu verwenden.
...
OpenDash Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

OpenDash Go funktioniert wie jeder andere Provider in OpenDash.

1. Du meldest dich bei **<a href={console}>OpenDash Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OpenDash Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OpenDash Go abonnieren.
:::
...
OpenDash Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner OpenDash Config verwendet das Format `opendash-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `opendash-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben OpenDash Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit OpenDash zu nutzen.
