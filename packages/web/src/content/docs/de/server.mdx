---
title: Server
description: Nutze den opendash-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `opendash serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein opendash-Client nutzen kann.
...
Der Benutzername ist standardmaessig `opendash`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `opendash serve` und `opendash web`.
...
Wenn du `opendash` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den opendash-Server, um opendash programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `opendash serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `opendash serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenDash-[IDE](/docs/ide)-Plugins.
...
Der opendash-Server stellt folgende APIs bereit.
