---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenDash zu erweitern.
---

Mit Plugins können Sie OpenDash erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenDash zu ändern.
...
- `.opendash/plugins/` – Plugins auf Projektebene
- `~/.config/opendash/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/opendash/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/opendash/opendash.json`)
2. Projektkonfiguration (`opendash.json`)
3. Globales Plugin-Verzeichnis (`~/.config/opendash/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.opendash/plugins/`)
...
OpenDash führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenDash-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenDash erweitern können.
...
:::note
Wenn Sie die OpenDash-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenDash `.env`-Dateien liest:
...
Plugins können OpenDash auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenDash aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenDash zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://opendashai.netlify.app/docs/sdk).
