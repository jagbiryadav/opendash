---
title: Complementos
description: Escriba sus propios complementos para extender OpenDash.
---

Los complementos le permiten extender OpenDash al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenDash.
...
- `.opendash/plugins/` - Complementos a nivel de proyecto
- `~/.config/opendash/plugins/` - Complementos globales
...
```json title="opendash.json"
{
  ...
  "plugin": ["opendash-helicone-session", "opendash-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/opendash/node_modules/`.
...
1. Configuración global (`~/.config/opendash/opendash.json`)
2. Configuración del proyecto (`opendash.json`)
3. Directorio global de complementos (`~/.config/opendash/plugins/`)
4. Directorio de complementos del proyecto (`.opendash/plugins/`)
...
```json title=".opendash/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenDash ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".opendash/plugins/my-plugin.ts"
...
```js title=".opendash/plugins/example.js"
...
- `client`: Un cliente SDK opendash para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar opendash.
...
```js title=".opendash/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "opendash"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenDash, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que opendash lea archivos `.env`:

```javascript title=".opendash/plugins/env-protection.js"
...
```javascript title=".opendash/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a opendash:

```ts title=".opendash/plugins/custom-tools.ts"
...
El ayudante `tool` crea una herramienta personalizada a la que opendash puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para opendash junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://opendashai.netlify.app/docs/sdk) para obtener más detalles.
...
```ts title=".opendash/plugins/compaction.ts"
import type { Plugin } from "@mimo-ai/plugin"
...
```ts title=".opendash/plugins/custom-compaction.ts"
import type { Plugin } from "@mimo-ai/plugin"
