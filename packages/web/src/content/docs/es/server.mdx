---
title: Servidor
description: Interactuar con el servidor opendash a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `opendash serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente opendash puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `opendash.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `opendash`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `opendash serve` como a `opendash web`.
...
Cuando ejecuta `opendash`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor opendash para interactuar con opendash mediante programación.
:::

Esta arquitectura permite que opendash admita múltiples clientes y le permite interactuar con opendash mediante programación.

Puede ejecutar `opendash serve` para iniciar un servidor independiente. Si tienes el
opendash TUI ejecutándose, `opendash serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenDash [IDE](/docs/ide).
...
El servidor opendash expone las siguientes API.
