...
Para depurar problemas con OpenDash, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/opendash/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\opendash\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `opendash --log-level DEBUG`.

---

## Almacenamiento

opendash almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/opendash/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\opendash`
...
OpenDash Desktop ejecuta un servidor OpenDash local (el sidecar `opendash-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `OpenDash` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/opendash/opendash.jsonc` (o `~/.config/opendash/opendash.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opendash/opendash.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opendash\opendash.jsonc`
...
OpenDash también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/opendash/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opendash\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.opendash/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que OpenDash pueda reconstruirlo.

1. Salga de OpenDash Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/opendash`
- **Linux**: eliminar `~/.cache/opendash` (o ejecutar `rm -rf ~/.cache/opendash`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\opendash`

3. Reinicie el escritorio OpenDash.

---

### Solucionar problemas de conexión del servidor

OpenDash Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `opendash.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio OpenDash requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de OpenDash.

---

### Notificaciones que no se muestran

OpenDash Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para OpenDash en la configuración de su sistema operativo, y
...
1. Salga del escritorio OpenDash.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio OpenDash):

- `opendash.settings.dat` (URL del servidor predeterminado de escritorio)
- `opendash.global.dat` y `opendash.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con OpenDash:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/opendash/opendash/issues**](https://github.com/opendash/opendash/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**opendashai.netlify.app/discord**](https://opendashai.netlify.app/discord)
...
3. Asegúrese de tener la última versión con `opendash upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `opendash models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/opendash
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\opendash`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. opendash instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/opendash
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\opendash`

2. Reinicie opendash para reinstalar los paquetes del proveedor más recientes.

Esto obligará a opendash a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
opendash detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
