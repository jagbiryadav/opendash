...
description: Cree herramientas que LLM pueda llamar en opendash.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de opendash como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.opendash/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/opendash/tools/`.
...
```ts title=".opendash/tools/database.ts" {1}
...
```ts title=".opendash/tools/math.ts"
import { tool } from "@mimo-ai/plugin"
...
```ts title=".opendash/tools/bash.ts"
import { tool } from "@mimo-ai/plugin"
...
```ts title=".opendash/tools/project.ts" {8}
import { tool } from "@mimo-ai/plugin"
...
```python title=".opendash/tools/add.py"
import sys
...
```ts title=".opendash/tools/python-add.ts" {10}
import { tool } from "@mimo-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".opendash/tools/add.py")
