---
title: CLI
description: Opciones y comandos de la CLI de OpenDash.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de OpenDash de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opendash
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con OpenDash mediante programación.
...
Inicie la interfaz de usuario del terminal OpenDash.
...
La CLI de OpenDash también tiene los siguientes comandos.

---

### agent

Administrar agentes para OpenDash.
...
Conecte una terminal a un servidor backend OpenDash que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opendash attach [url]
```

Esto permite usar TUI con un backend remoto OpenDash. Por ejemplo:
...
OpenDash funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opendash auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opendash/auth.json`.

```bash
opendash auth login
```

Cuando se inicia OpenDash, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en OpenDash.
...
Ejecute opendash en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `opendash serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| `--attach`   |       | Adjuntar a un servidor opendash en ejecución (por ejemplo, http://localhost:4096)   |
...
Inicie un servidor OpenDash sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
opendash serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad opendash sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opendash`).
...
Enumere todas las sesiones OpenDash.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones OpenDash.
...
Importe datos de sesión desde un archivo JSON o una URL compartida OpenDash.

```bash
opendash import <file>
```

Puede importar desde un archivo local o una URL compartida OpenDash.
...
Inicie un servidor OpenDash sin cabeza con una interfaz web.

```bash
opendash web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a OpenDash a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opendash`).
...
Desinstale OpenDash y elimine todos los archivos relacionados.
...
Actualiza opendash a la última versión o a una versión específica.
...
La CLI de OpenDash toma las siguientes banderas globales.
...
OpenDash se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `opendash`) |
