...
Las habilidades del agente permiten a OpenDash descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenDash busca estas ubicaciones:

- Configuración del proyecto: `.opendash/skills/<name>/SKILL.md`
- Configuración global: `~/.config/opendash/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenDash sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.opendash/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.opendash/skills/git-release/SKILL.md` así:
...
OpenDash enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `opendash.json`:
...
**Para agentes integrados** (en `opendash.json`):
