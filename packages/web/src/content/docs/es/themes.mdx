...
Con OpenDash puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenDash usa nuestro propio tema `opendash`.
...
OpenDash viene con varios temas integrados.
...
- Quiere que OpenDash coincida con la apariencia de su terminal
...
OpenDash admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/opendash/themes/*.json` o `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.opendash/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.opendash/themes/*.json`
