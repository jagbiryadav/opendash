---
title: Proveedores
description: Usando cualquier proveedor LLM en OpenDash.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenDash usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración OpenDash.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu OpenDash
configuración.
...
OpenDash Zen es una lista de modelos proporcionados por el equipo OpenDash que han sido
probado y verificado para funcionar bien con OpenDash. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenDash Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opendash y diríjase a [opendashai.netlify.app/auth](https://opendashai.netlify.app/auth).
...
Funciona como cualquier otro proveedor en OpenDash y su uso es completamente opcional.

---

## OpenDash Go

OpenDash Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de OpenDash que han sido
probado y verificado para funcionar bien con OpenDash.

1. Ejecute el comando `/connect` en TUI, seleccione `OpenDash Go` y diríjase a [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Para usar Amazon Bedrock con OpenDash:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta opendash:
   ...
   Para una configuración persistente o específica del proyecto, utilice `opendash.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en OpenDash no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que opendash funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración opendash.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `OpenDash`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación opendash.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
OpenDash utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear OpenDash
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `opendash.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `opendash.json`:
...
Para utilizar su suscripción GitHub Copilot con opendash:
...
Para utilizar Google Vertex AI con OpenDash:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta opendash.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de opendash, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación OpenDash enumerada como una sesión separada.
...
Puede configurar opendash para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar opendash para usar modelos locales a través de LM Studio.
...
Puedes configurar opendash para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para OpenDash. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opendash) para obtener más detalles.
:::
...
Para usar Ollama Cloud con OpenDash:
...
3. Copie la clave API para usarla en OpenDash.
...
6. **Importante**: Antes de usar modelos en la nube en OpenDash, debe obtener la información del modelo localmente:
...
OpenDash Zen es una lista de modelos probados y verificados proporcionada por el equipo OpenDash. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>OpenDash Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **OpenDash Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave OpenDash API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración opendash.
...
5. También puedes personalizarlos a través de tu configuración opendash. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con OpenDash:
...
También puede personalizar modelos a través de su configuración opendash. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con opendash. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `opendash.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a OpenDash comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `opendash auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración opendash y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración opendash.
