---
title: プラグイン
description: OpenDash を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで OpenDash を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、OpenDash のデフォルトの動作を変更したりできます。
...
- `.opendash/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/opendash/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/opendash/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/opendash/opendash.json`)
2. プロジェクト設定 (`opendash.json`)
3. グローバルプラグインディレクトリ (`~/.config/opendash/plugins/`)
4. プロジェクトプラグインディレクトリ (`.opendash/plugins/`)
...
OpenDash は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための OpenDash SDK クライアント。
...
ここでは、OpenDash を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://opendashai.netlify.app/docs/sdk) を参照してください。
...
OpenDash が `.env` ファイルを読み取らないようにします。
...
プラグインは OpenDash にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、OpenDash が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して OpenDash で使用できます。
