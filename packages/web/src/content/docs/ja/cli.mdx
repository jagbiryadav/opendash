---
title: CLI
description: OpenDash CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenDash CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
opendash
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、OpenDash をプログラム的に操作できるようになります。
...
OpenDash ターミナルユーザーインターフェイスを開始します。
...
OpenDash CLI には次のコマンドもあります。

---

### agent

OpenDash のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の OpenDash バックエンドサーバーにターミナルを接続します。

```bash
opendash attach [url]
```

これにより、リモート OpenDash バックエンドで TUI を使用できるようになります。例えば：
...
OpenDash は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`opendash auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/opendash/auth.json` に保存されます。

```bash
opendash auth login
```

OpenDash が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを OpenDash で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで opendash を実行します。
...
実行中の `opendash serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| `--attach`   |          | 実行中の opendash サーバー (http://localhost:4096 など) に接続します。                    |
...
API アクセスのためにヘッドレス OpenDash サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
opendash serve
```

これにより、TUI インターフェイスを使用せずに opendash 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opendash`)。
...
OpenDash セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは OpenDash 共有 URL からセッションデータをインポートします。

```bash
opendash import <file>
```

ローカルファイルまたは OpenDash 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス OpenDash サーバーを起動します。

```bash
opendash web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて OpenDash にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opendash`)。
...
OpenDash をアンインストールし、関連ファイルをすべて削除します。
...
opendash を最新バージョンまたは特定のバージョンに更新します。
...
opendash CLI は次のグローバルフラグを受け取ります。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `opendash`) をオーバーライドします。 |
