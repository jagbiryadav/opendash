...
With OpenDash you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenDash uses our own `opendash` theme.
...
OpenDash comes with several built-in themes.
...
- Want OpenDash to match their terminal's appearance
...
OpenDash supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/opendash/themes/*.json` or `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.opendash/themes/*.json`
4. **Current working directory** - Defined in `./.opendash/themes/*.json`
