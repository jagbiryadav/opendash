---
title: TUI
description: Using the OpenDash terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenDash provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenDash starts the TUI for the current directory.
...
When using the OpenDash TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenDash. Allows you to select from available providers and add their API keys.
...
Exit OpenDash. _Aliases_: `/quit`, `/q`
...
This is separate from `opendash.json`, which configures server/runtime behavior.
