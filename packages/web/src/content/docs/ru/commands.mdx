...
Создайте `.opendash/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию opendash или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем opendash [config](/docs/config):
...
- Глобальный: `~/.config/opendash/commands/`
- Для каждого проекта: `.opendash/commands/`
...
opendash включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
