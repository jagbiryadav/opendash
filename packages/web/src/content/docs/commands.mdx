...
Create `.opendash/commands/test.md`:
...
You can add custom commands through the OpenDash config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenDash [config](/docs/config):
...
opendash includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
