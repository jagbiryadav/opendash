---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenDash.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenDash bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenDash-konfigurasjonen.
...
i `~/.local/share/opendash/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenDash
konfig.
...
OpenDash Zen er en liste over modeller levert av OpenDash-teamet som har vært
testet og verifisert for å fungere godt med OpenDash. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenDash Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenDash Zen` og gå til [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Det fungerer som alle andre leverandører i OpenDash og er helt valgfritt å bruke.

---

## OpenDash Go

OpenDash Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenDash-teamet som har vært
testet og verifisert for å fungere godt med OpenDash.

1. Kjør kommandoen `/connect` i TUI, velg `OpenDash Go`, og gå til [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Slik bruker du Amazon Bedrock med OpenDash:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenDash:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `opendash.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i opendash støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenDash skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenDash-konfigurasjonen.
...
for å lagre token i OpenDash auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenDash bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenDash
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`opendash.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med OpenDash:
...
Slik bruker du Google Vertex AI med OpenDash:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenDash.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenDash, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session) plugin for automatisk å logge hver OpenDash-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenDash-samtale oppført som en separat økt.
...
Du kan konfigurere OpenDash for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenDash for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenDash for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenDash. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opendash) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenDash:
...
3. Kopier API-nøkkelen for bruk i OpenDash.
...
6. **Viktig**: Før du bruker skymodeller i OpenDash, må du hente modellinformasjonen lokalt:
...
OpenDash Zen er en liste over testede og verifiserte modeller levert av OpenDash-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenDash Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenDash Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenDash API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenDash-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenDash-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenDash:
...
Du kan også tilpasse modeller gjennom OpenDash-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenDash. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `opendash.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenDash forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `opendash auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenDash-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenDash-konfigurasjonen.
