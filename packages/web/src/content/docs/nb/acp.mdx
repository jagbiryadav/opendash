---
title: ACP-støtte
description: Bruk opendash i en hvilken som helst ACP-kompatibel editor.
---

OpenDash støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke OpenDash via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `opendash acp`.

Kommandoen starter OpenDash som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'OpenDash'-agenten i AI Chat-agentvelgeren.
...
For å bruke OpenDash som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke OpenDash som ACP-agent for chat.
...
OpenDash fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i OpenDash-konfigurasjonen
