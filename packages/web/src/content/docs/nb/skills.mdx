...
Agentferdigheter lar OpenDash oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
opendash søker etter disse stedene:

- Prosjektkonfigurasjon: `.opendash/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/opendash/skills/<name>/SKILL.md`
...
For prosjektlokale stier går opendash opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.opendash/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.opendash/skills/git-release/SKILL.md` slik:
...
opendash viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `opendash.json`:
...
**For innebygde agenter** (i `opendash.json`):
