...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i OpenDash-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i OpenDash lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom OpenDash-konfigurasjonen.
...
OpenDash kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.opendash/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `opendash.json` konfigurasjonsfil:
...
- Globalt: `~/.config/opendash/modes/`
- Prosjekt: `.opendash/modes/`
...
Hvis ingen temperatur er spesifisert, bruker OpenDash modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenDash-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.opendash/modes/` for prosjektspesifikke moduser eller `~/.config/opendash/modes/` for globale moduser:
