...
Det er to typer agenter i OpenDash; primære agenter og underagenter.
...
OpenDash kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenDash kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenDash kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `opendash.json`:
...
- Globalt: `~/.config/opendash/agents/`
- Per prosjekt: `.opendash/agents/`
...
Hvis ingen temperatur er spesifisert, bruker OpenDash modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenDash-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenDash-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenDash Zen](/docs/zen), vil du bruke `opendash/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `opendash models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/opendash/opendash).
:::
