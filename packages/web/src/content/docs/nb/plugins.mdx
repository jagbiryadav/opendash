...
description: Skriv dine egne programtillegg for å utvide OpenDash.
---

Plugins lar deg utvide OpenDash ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenDash.
...
- `.opendash/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/opendash/node_modules/`.
...
1. Global konfigurasjon (`~/.config/opendash/opendash.json`)
2. Prosjektkonfigurasjon (`opendash.json`)
3. Global plugin-katalog (`~/.config/opendash/plugins/`)
4. Prosjektpluginkatalog (`.opendash/plugins/`)
...
OpenDash kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenDash SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenDash.
...
:::note
Hvis du bruker OpenDash-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenDash fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenDash:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenDash kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenDash sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://opendashai.netlify.app/docs/sdk) for detaljer.
