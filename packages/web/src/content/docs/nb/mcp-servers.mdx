...
Du kan legge til eksterne verktøy i OpenDash ved å bruke _Model Context Protocol_ eller MCP. OpenDash støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenDash Config](https://opendashai.netlify.app/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="opendash.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/opendash`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
OpenDash håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenDash:
...
Hvis serveren krever autentisering, vil OpenDash be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opendash mcp auth <server-name>`.
...
```bash
opendash mcp auth my-oauth-server
```
...
```bash
opendash mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenDash lagre tokens sikkert i `~/.local/share/opendash/mcp-auth.json`.
...
opendash mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenDash, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenDash-konfigurasjonen som alle andre verktøy.
...
```json title="opendash.json" {11, 14-18}
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
opendash mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenDash til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenDash-samtale oppført som en separat økt.
