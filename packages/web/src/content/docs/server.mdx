---
title: Server
description: Interact with opendash server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `opendash serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opendash client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `opendash.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opendash`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `opendash serve` and `opendash web`.
...
When you run `opendash` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the opendash server to interact with opendash programmatically.
:::

This architecture lets opendash support multiple clients and allows you to interact with opendash programmatically.

You can run `opendash serve` to start a standalone server. If you have the
opendash TUI running, `opendash serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenDash [IDE](/docs/ide) plugins.
...
The opendash server exposes the following APIs.
