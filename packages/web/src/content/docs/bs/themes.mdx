...
U OpenDash mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenDash koristi nasu `opendash` temu.
...
OpenDash dolazi sa vise ugradenih tema.
...
- Zele da OpenDash odgovara izgledu njihovog terminala
...
OpenDash podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/opendash/themes/*.json` ili `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.opendash/themes/*.json`
4. **Trenutni radni direktorij** - `./.opendash/themes/*.json`
