...
Možete dodati vanjske alate u OpenDash koristeći _Model Context Protocol_, ili MCP. OpenDash podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenDash Config](https://opendashai.netlify.app/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/opendash`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenDash automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenDash će:
...
Ako server zahtijeva autentifikaciju, OpenDash će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opendash mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenDash sigurno čuva tokene u `~/.local/share/opendash/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenDash, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenDash konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenDash sa vašim Sentry nalogom.
