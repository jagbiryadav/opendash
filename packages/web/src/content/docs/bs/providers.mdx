---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenDash.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenDash koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenDash konfiguraciji.
...
u `~/.local/share/opendash/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenDash
config.
...
OpenDash Zen je lista modela koje je obezbedio OpenDash tim koji su bili
testirano i potvrđeno da dobro radi sa OpenDash. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenDash Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite opendash i idite na [opendashai.netlify.app/auth](https://opendashai.netlify.app/auth).
...
Radi kao i svaki drugi provajder u OpenDash i potpuno je opcionalan za korištenje.

---

## OpenDash Go

OpenDash Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenDash tim i koji su testirani i verificirani da dobro rade s OpenDash-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenDash Go` i idite na [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Da biste koristili Amazon Bedrock s OpenDash:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete opendash:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `opendash.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenDash.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi opendash ispravno radio.
   :::
...
Također možete dodati modele kroz svoju opendash konfiguraciju.
...
da pohrani token u opendash auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenDash koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenDash
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`opendash.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `opendash.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s opendash:
...
Za korištenje Google Vertex AI s OpenDash:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće opendash.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem opendasha, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session) da automatski prijavite svaki OpenDash razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenDash razgovor naveden kao zasebna sesija.
...
Možete konfigurirati opendash za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati opendash za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati opendash za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenDash. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opendash) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenDash:
...
3. Kopirajte API ključ za korištenje u OpenDash.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenDash, morate lokalno povući informacije o modelu:
...
OpenDash Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenDash tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenDash Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenDash Zen**.

```txt
   /connect
```

3. Unesite svoj OpenDash API ključ.
...
Također možete dodati dodatne modele putem vaše opendash konfiguracije.
...
5. Također ih možete prilagoditi putem vaše opendash konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju opendash konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s opendash-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `opendash.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenDash da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `opendash auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenDash konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj opendash konfiguraciji.
