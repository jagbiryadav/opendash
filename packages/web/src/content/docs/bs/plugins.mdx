---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenDash.
---

Dodaci vam omogućavaju da proširite OpenDash spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenDash.
...
- `.opendash/plugins/` - Dodaci na nivou projekta
- `~/.config/opendash/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/opendash/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/opendash/opendash.json`)
2. Konfiguracija projekta (`opendash.json`)
3. Globalni direktorij dodataka (`~/.config/opendash/plugins/`)
4. Direktorij dodataka projekta (`.opendash/plugins/`)
...
OpenDash pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenDash.
...
Spriječite opendash da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u opendash:
...
Pomoćnik `tool` kreira prilagođeni alat koji opendash može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za opendash zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://opendashai.netlify.app/docs/sdk) za detalje.
