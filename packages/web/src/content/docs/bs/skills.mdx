...
Agent skills omogucavaju OpenDash da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenDash pretrazuje ove lokacije:

- Konfiguracija projekta: `.opendash/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/opendash/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenDash ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.opendash/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.opendash/skills/git-release/SKILL.md` ovako:
...
OpenDash navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `opendash.json`:
...
**Za ugradene agente** (u `opendash.json`):
