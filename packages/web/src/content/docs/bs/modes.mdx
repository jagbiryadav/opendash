...
ove ili konfigurirajte svoje putem opendash config.
...
opendash dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.opendash/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `opendash.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/opendash/modes/`
- Projekat: `.opendash/modes/`
...
Ako temperatura nije navedena, opendash koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.opendash/modes/` za specifične načine rada ili `~/.config/opendash/modes/` za globalne načine:
