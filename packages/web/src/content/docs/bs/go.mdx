...
OpenDash Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
OpenDash Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u OpenDash-u. Pretplatite se na OpenDash Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili OpenDash.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa OpenDash-om.
:::
...
OpenDash Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

OpenDash Go radi kao bilo koji drugi provajder u OpenDash-u.

1. Prijavite se na **<a href={console}>OpenDash Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `OpenDash Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na OpenDash Go.
:::
...
OpenDash Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj OpenDash konfiguraciji
koristi format `opendash-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`opendash-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo OpenDash Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz OpenDash koristite i bilo kojeg drugog provajdera.
