...
Postoje dvije vrste agenata u OpenDash: primarni agenti i podagenti.
...
OpenDash dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenDash dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenDash dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `opendash.json`:
...
- Globalno: `~/.config/opendash/agents/`
- Po projektu: `.opendash/agents/`
...
Ako temperatura nije navedena, OpenDash koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenDash konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenDash konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenDash Zen](/docs/zen), koristili biste `opendash/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `opendash models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/opendash/opendash).
:::
