...
Da biste otklonili probleme s OpenDash, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\opendash\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `opendash --log-level DEBUG`.

---

## Pohrana

OpenDash pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opendash/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\opendash`
...
OpenDash Desktop pokreće lokalni OpenDash server (`opendash-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenDash` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/opendash/opendash.jsonc` (ili `~/.config/opendash/opendash.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opendash/opendash.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opendash\opendash.jsonc`
...
OpenDash također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/opendash/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opendash\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenDash mogao ponovo izgraditi.

1. Potpuno zatvorite OpenDash Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/opendash`
- **Linux**: obrišite `~/.cache/opendash` (ili pokrenite `rm -rf ~/.cache/opendash`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\opendash`

3. Ponovo pokrenite OpenDash Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenDash Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `opendash.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, OpenDash Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenDash karakteristikama.

---

### Obavještenja se ne prikazuju

OpenDash Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenDash u postavkama vašeg OS-a, i
...
1. Zatvorite OpenDash Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenDash Desktop aplikacije):

- `opendash.settings.dat` (URL zadanog servera za desktop)
- `opendash.global.dat` i `opendash.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `opendash models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\opendash`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenDash dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\opendash`

2. Ponovo pokrenite OpenDash da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenDash da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenDash će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
