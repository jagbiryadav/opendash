---
title: Server
description: Komunicirajte s opendash serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `opendash serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opendash klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `opendash.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opendash`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `opendash serve` i za `opendash web`.
...
Kada pokrenete `opendash`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite opendash server za programsku interakciju sa opendash-om.
:::

Ova arhitektura omogucava opendash podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `opendash serve` da startate standalone server. Ako je opendash TUI vec pokrenut, `opendash serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenDash [IDE](/docs/ide) pluginovi.
...
opendash server izlaže sljedece API-je.
