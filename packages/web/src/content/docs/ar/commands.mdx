...
أنشئ الملف `.opendash/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenDash أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenDash:
...
- على مستوى النظام: `~/.config/opendash/commands/`
- لكل مشروع: `.opendash/commands/`
...
يتضمن opendash عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
