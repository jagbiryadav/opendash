...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات opendash. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في opendash تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات opendash.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.opendash/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `opendash.json` لديك:
...
- عالمي: `~/.config/opendash/modes/`
- للمشروع: `.opendash/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم opendash القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات opendash العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.opendash/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/opendash/modes/` لأوضاع عالمية:
