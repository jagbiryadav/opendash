---
title: CLI
description: أوامر وخيارات CLI في OpenDash.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ OpenDash افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
opendash
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع OpenDash برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ OpenDash.
...
يتضمن CLI الخاص بـ OpenDash أيضا الأوامر التالية.

---

### agent

إدارة وكلاء OpenDash.
...
إرفاق terminal بخادم الواجهة الخلفية لـ OpenDash قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opendash attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ OpenDash تعمل عن بعد. على سبيل المثال:
...
يعتمد OpenDash على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `opendash auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opendash/auth.json`.

```bash
opendash auth login
```

عند تشغيل OpenDash يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في OpenDash.
...
تشغيل opendash في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `opendash serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| `--attach`   |         | الإرفاق بخادم opendash قيد التشغيل (مثل http://localhost:4096)    |
...
بدء خادم OpenDash بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
opendash serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف opendash دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opendash`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات OpenDash لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة OpenDash.

```bash
opendash import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة OpenDash.
...
بدء خادم OpenDash بدون واجهة مع واجهة ويب.

```bash
opendash web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى OpenDash عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opendash`).
...
إلغاء تثبيت OpenDash وإزالة جميع الملفات ذات الصلة.
...
تحديث opendash إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر opendash الخيارات العامة التالية.
...
يمكن تهيئة OpenDash باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `opendash`) |
