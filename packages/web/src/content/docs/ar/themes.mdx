...
مع OpenDash يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenDash سمتنا `opendash`.
...
يأتي OpenDash مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenDash مظهر terminal لديهم
...
يدعم OpenDash نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/opendash/themes/*.json` أو `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.opendash/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.opendash/themes/*.json`
