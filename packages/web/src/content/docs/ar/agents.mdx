...
يوجد نوعان من الوكلاء في OpenDash: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي OpenDash مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي OpenDash مع وكيلين فرعيين مدمجين: **General** و **Explore**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي OpenDash مع وكيلين أساسيين مدمجين ووكيلين فرعيين مدمجين.
...
هيّئ الوكلاء في ملف إعدادات `opendash.json`:
...
- عام: `~/.config/opendash/agents/`
- لكل مشروع: `.opendash/agents/`
...
إذا لم يتم تحديد temperature، يستخدم OpenDash القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات OpenDash العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات OpenDash الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [OpenDash Zen](/docs/zen)، فستستخدم `opendash/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `opendash models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/opendash/opendash).
:::
