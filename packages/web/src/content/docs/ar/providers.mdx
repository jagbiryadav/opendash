---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenDash.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenDash ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenDash.
...
في `~/.local/share/opendash/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenDash.
...
OpenDash Zen هي قائمة نماذج يوفّرها فريق OpenDash وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenDash. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenDash Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opendash، ثم انتقل إلى [opendashai.netlify.app/auth](https://opendashai.netlify.app/auth).
...
يعمل مثل أي مزوّد آخر في OpenDash واستخدامه اختياري بالكامل.

---

## OpenDash Go

OpenDash Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenDash، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenDash.

1. شغّل الأمر `/connect` في TUI، واختر `OpenDash Go`، ثم انتقل إلى [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
لاستخدام Amazon Bedrock مع OpenDash:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل opendash:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `opendash.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenDash غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل opendash بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات opendash.
...
تخزين الرمز في مساحة تخزين مصادقة opendash.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenDash نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenDash
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`opendash.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع opendash:
...
لاستخدام Google Vertex AI مع OpenDash:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر opendash، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session) لتسجيل كل محادثة OpenDash تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenDash مدرجة كجلسة مستقلة.
...
يمكنك ضبط opendash لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط opendash لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط opendash لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenDash. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opendash) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenDash:
...
3. انسخ مفتاح API لاستخدامه في OpenDash.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenDash، يجب سحب معلومات النموذج محليا:
...
OpenDash Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenDash. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenDash Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenDash Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenDash.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات opendash.
...
5. يمكنك أيضا تخصيصها عبر إعدادات opendash. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات opendash. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع opendash. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `opendash.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenDash بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `opendash auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات opendash وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات opendash.
