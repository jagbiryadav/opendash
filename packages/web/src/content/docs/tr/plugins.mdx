---
title: Eklentiler
description: OpenDash'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenDash'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenDash'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.opendash/plugins/` - Proje düzeyinde eklentiler
- `~/.config/opendash/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/opendash/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/opendash/opendash.json`)
2. Project config (`opendash.json`)
3. Global eklenti dizini (`~/.config/opendash/plugins/`)
4. Proje eklenti dizini (`.opendash/plugins/`)
...
OpenDash bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenDash SDK istemcisi.
...
OpenDash'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenDash masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenDash'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenDash'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenDash'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenDash'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://opendashai.netlify.app/docs/sdk) bakın.
