...
opendash ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\opendash\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `opendash --log-level DEBUG`.

---

## Depolama

opendash, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/opendash/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\opendash` yapistirin
...
opendash Desktop arka planda yerel bir opendash sunucusu (`opendash-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `opendash` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/opendash/opendash.jsonc` (veya `~/.config/opendash/opendash.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opendash/opendash.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opendash\opendash.jsonc` yapistirin
...
opendash diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/opendash/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opendash\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), opendash'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. opendash Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/opendash`
- **Linux**: `~/.cache/opendash` dizinini silin (veya `rm -rf ~/.cache/opendash` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\opendash` yapistirin

3. opendash Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

opendash Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`opendash.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta opendash Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, opendash ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

opendash Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda opendash icin bildirimler etkinse
...
1. opendash Desktop'u kapatin
2. Su dosyalari bulun ve silin (opendash Desktop uygulama veri dizininde yer alirlar):

- `opendash.settings.dat` (desktop varsayilan sunucu URL'si)
- `opendash.global.dat` ve `opendash.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
opendash ile ilgili bir sorun yasiyorsaniz:
...
3. `opendash upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `opendash models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\opendash`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. opendash, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\opendash`

2. En guncel provider paketlerini yeniden kurmak icin opendash'u yeniden baslatin

Bu, opendash'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
opendash, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
