...
OpenDash Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
OpenDash Go şu anda beta olarak mevcut.
:::

Go, OpenDash'daki diğer sağlayıcılar gibi çalışır. OpenDash Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenDash'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenDash Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OpenDash Go, OpenDash'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenDash Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenDash Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OpenDash Go'ya abone olabilir.
:::
...
OpenDash Go aşağıdaki limitleri içerir:
...
OpenDash yapılandırmanızdaki [model id](/docs/config/#models) formatı `opendash-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `opendash-go/kimi-k2.6` kullanmalısınız.
...
OpenDash Go'yu şu amaçlarla oluşturduk:
...
4. OpenDash ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
