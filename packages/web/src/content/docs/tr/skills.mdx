...
Ajan becerileri, opendash'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
opendash su konumlari tarar:

- Proje konfigurasyonu: `.opendash/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/opendash/skills/<name>/SKILL.md`
...
Proje yerel yollarinda opendash, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.opendash/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.opendash/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
opendash, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`opendash.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`opendash.json` icinde):
