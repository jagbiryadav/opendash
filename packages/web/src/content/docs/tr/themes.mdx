...
opendash ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak opendash kendi `opendash` temasını kullanır.
...
opendash birden fazla yerleşik temayla gelir.
...
- opendash'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
opendash, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/opendash/themes/*.json` veya `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.opendash/themes/*.json`
4. **Geçerli çalışma dizini** - `./.opendash/themes/*.json`
