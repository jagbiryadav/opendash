...
description: opendash dile özgü biçimlendiriciler kullanır.
---

opendash, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

opendash, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, opendash bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

opendash bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri opendash yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
