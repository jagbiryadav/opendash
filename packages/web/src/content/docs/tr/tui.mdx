---
title: TUI
description: OpenDash terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenDash, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

OpenDash'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
OpenDash TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
OpenDash'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
OpenDash'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `opendash.json` dosyasından ayrıdır.
