...
_Model Bağlam Protokolü_ veya MCP kullanarak opendash'a harici araçlar ekleyebilirsiniz. opendash hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [opendash Config](https://opendashai.netlify.app/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/opendash` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
opendash, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde opendash şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa opendash, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `opendash mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından opendash, belirteçleri `~/.local/share/opendash/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra opendash'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi opendash yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve opendash'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
