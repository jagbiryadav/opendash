---
title: ACP Desteği
description: opendash'u ACP uyumlu herhangi bir editörde kullanın.
---

opendash, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
opendash'u ACP aracılığıyla kullanmak için düzenleyicinizi `opendash acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, opendash'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'OpenDash' ajanını kullanın.
...
opendash'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı opendash'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
opendash, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- opendash yapılandırmanızda yapılandırılmış MCP sunucuları
