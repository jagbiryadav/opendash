---
title: Sağlayıcılar
description: opendash'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

opendash, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı opendash yapılandırmanızda yapılandırın.
...
`~/.local/share/opendash/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları opendash'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenDash Zen, opendash ekibi tarafından sağlanan modellerin bir listesidir.
opendash ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenDash Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, opendash'u seçin ve [opendashai.netlify.app/auth](https://opendashai.netlify.app/auth)'ye gidin.
...
opendash'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenDash Go

OpenDash Go, opendash ile iyi çalıştığı test edilmiş ve doğrulanmış, opendash ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenDash Go`'yu seçin ve [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen) adresine gidin.
...
Amazon Bedrock'u opendash ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   opendash'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `opendash.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi opendash'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   opendash'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in opendash auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
opendash, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. opendash'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`opendash.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`opendash.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi opendash'la kullanmak için:
...
Google Vertex AI'yi opendash ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları opendash çalıştırılırken ayarlayın.
...
Helicone'un opendash aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her opendash görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her opendash görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
opendash'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla opendash'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla opendash'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini opendash için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opendash)'a bakın.
:::
...
Ollama Cloud'u opendash ile kullanmak için:
...
3. opendash'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: opendash'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenDash Zen, opendash ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenDash Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenDash Zen**'i arayın.

   ```txt
   /connect
   ```

3. opendash API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   opendash yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca opendash yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri opendash yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı opendash ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `opendash.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları opendash'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `opendash auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için opendash yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, opendash yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
