...
opendash'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
opendash, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
opendash, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

opendash iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `opendash.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse opendash modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global opendash yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
opendash yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [OpenDash Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opendash/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `opendash models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/opendash/opendash).
:::
