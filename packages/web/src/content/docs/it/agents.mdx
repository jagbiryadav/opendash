...
In OpenDash esistono due tipi di agenti: agenti primari e subagenti.
...
OpenDash include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenDash include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

OpenDash include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `opendash.json`:
...
- Globale: `~/.config/opendash/agents/`
- Per progetto: `.opendash/agents/`
...
Se non specifichi una temperature, OpenDash usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenDash sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenDash usa il formato `provider/model-id`. Per esempio, se stai usando [OpenDash Zen](/docs/zen), useresti `opendash/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `opendash models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/opendash/opendash).
:::
