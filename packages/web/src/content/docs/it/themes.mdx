...
Con OpenDash puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OpenDash usa il tema `opendash`.
...
OpenDash include diversi temi integrati.
...
- Vuole che OpenDash corrisponda all'aspetto del terminale
...
OpenDash supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/opendash/themes/*.json` o `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.opendash/themes/*.json`
4. **Directory di lavoro corrente** - in `./.opendash/themes/*.json`
