---
title: Plugin
description: Scrivi plugin per estendere OpenDash.
---

I plugin ti permettono di estendere OpenDash agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenDash.
...
- `.opendash/plugins/` - plugin a livello progetto
- `~/.config/opendash/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/opendash/node_modules/`.
...
1. Config globale (`~/.config/opendash/opendash.json`)
2. Config di progetto (`opendash.json`)
3. Directory plugin globale (`~/.config/opendash/plugins/`)
4. Directory plugin di progetto (`.opendash/plugins/`)
...
OpenDash esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di opendash per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere opendash.
...
:::note
Se usi l'app desktop di OpenDash, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a opendash di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a opendash:
...
L'helper `tool` crea uno strumento personalizzato che opendash può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in opendash insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://opendashai.netlify.app/docs/sdk) per i dettagli.
