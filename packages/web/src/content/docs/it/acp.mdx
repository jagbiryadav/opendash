---
title: Supporto ACP
description: Usa OpenDash in qualunque editor compatibile con ACP.
---

OpenDash supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare OpenDash via ACP, configura l'editor in modo che esegua il comando `opendash acp`.

Il comando avvia OpenDash come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'OpenDash' nel selettore agenti di AI Chat.
...
Per usare OpenDash come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare OpenDash come agente ACP per la chat.
...
OpenDash funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione OpenDash
