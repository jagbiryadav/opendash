---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenDash.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenDash fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenDash avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenDash, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenDash. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenDash. _Alias_: `/quit`, `/q`
...
Questo è separato da `opendash.json`, che configura il comportamento di server/runtime.
