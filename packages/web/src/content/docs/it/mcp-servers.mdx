...
Puoi aggiungere strumenti esterni a OpenDash usando il _Model Context Protocol_, o MCP. OpenDash supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenDash Config](https://opendashai.netlify.app/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/opendash`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
OpenDash gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenDash:
...
Se il server richiede autenticazione, OpenDash ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opendash mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenDash memorizzera' i token in modo sicuro in `~/.local/share/opendash/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenDash insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenDash come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenDash al tuo account Sentry.
