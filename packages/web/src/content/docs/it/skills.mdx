...
Le skill degli agenti permettono a OpenDash di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenDash cerca in queste posizioni:

- Config di progetto: `.opendash/skills/<name>/SKILL.md`
- Config globale: `~/.config/opendash/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenDash risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.opendash/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/opendash/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.opendash/skills/git-release/SKILL.md` cosi':
...
OpenDash elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `opendash.json`:
...
**Per agenti integrati** (in `opendash.json`):
