...
ด้วย OpenDash คุณสามารถเลือกจากหนึ่งในธีมที่มีอยู่แล้วภายใน ใช้ธีมที่ปรับให้เข้ากับธีม terminal ของคุณ หรือกำหนดธีมที่คุณกำหนดเองได้

ตามค่าเริ่มต้น OpenDash จะใช้ธีม `opendash` ของเราเอง
...
OpenDash มาพร้อมกับธีมในตัวหลายธีม

| ชื่อ                   | คำอธิบาย                                                            |
| ---------------------- | ------------------------------------------------------------------- |
| `system`               | ปรับให้เข้ากับสีพื้นหลังของ terminal ของคุณ                         |
...
ธีม `system` ได้รับการออกแบบให้ปรับให้เข้ากับโทนสีของเครื่อง terminal ของคุณโดยอัตโนมัติ ไม่เหมือนกับธีมดั้งเดิมที่ใช้สีคงที่ ธีม _system_:
...
- ต้องการให้ OpenDash ตรงกับรูปลักษณ์ของ terminal
...
OpenDash รองรับระบบธีมที่ใช้ JSON ที่ยืดหยุ่น ซึ่งช่วยให้ผู้ใช้สามารถสร้างและปรับแต่งธีมได้อย่างง่ายดาย
...
2. **ไดเรกทอรีกำหนดค่าผู้ใช้** - กำหนดใน `~/.config/opendash/themes/*.json` หรือ `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **ไดเรกทอรีรากของโครงการ** - กำหนดไว้ใน `<project-root>/.opendash/themes/*.json`
4. **ไดเร็กทอรีการทำงานปัจจุบัน** - กำหนดใน `./.opendash/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/opendash/themes
vim ~/.config/opendash/themes/my-theme.json
```

และสำหรับธีมเฉพาะโครงการ

```bash no-frame
mkdir -p .opendash/themes
vim .opendash/themes/my-theme.json
```
...
{
  "$schema": "https://opendashai.netlify.app/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
