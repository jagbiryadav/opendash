---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenDash.
---

Les plugins vous permettent d'étendre OpenDash en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenDash.
...
- `.opendash/plugins/` - Plugins au niveau du projet
- `~/.config/opendash/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/opendash/node_modules/`.
...
1. Configuration globale (`~/.config/opendash/opendash.json`)
2. Configuration du projet (`opendash.json`)
3. Répertoire global des plugins (`~/.config/opendash/plugins/`)
4. Répertoire des plugins du projet (`.opendash/plugins/`)
...
OpenDash exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK opendash pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre opendash.
...
:::note
Si vous utilisez l'application de bureau OpenDash, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez opendash de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à opendash :
...
L'assistant `tool` crée un outil personnalisé que opendash peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour opendash aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://opendashai.netlify.app/docs/sdk) pour plus de détails.
