...
Il existe deux types d'agents dans OpenDash ; agents primaires et sous-agents.
...
OpenDash est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
OpenDash est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

OpenDash est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `opendash.json` :
...
- Global : `~/.config/opendash/agents/`
- Par projet : `.opendash/agents/`

```markdown title="~/.config/opendash/agents/review.md"
---
...
Si aucune température n'est spécifiée, OpenDash utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale OpenDash et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration OpenDash utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenDash Zen](/docs/zen), vous utiliserez `opendash/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="opendash.json" {3-6,9-12}
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "tools": {
    "write": true,
...
```json title="opendash.json" {3-5,8-10}
{
  "$schema": "https://opendashai.netlify.app/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `opendash models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/opendash/opendash).
:::
...
```markdown title="~/.config/opendash/agents/docs-writer.md"
---
...
```markdown title="~/.config/opendash/agents/security-auditor.md"
---
