---
title: Serveur
description: Interagissez avec le serveur opendash via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `opendash serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client opendash peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `opendash.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `opendash`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `opendash serve` et à `opendash web`.
...
Lorsque vous exécutez `opendash`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur opendash pour interagir avec opendash par programmation.
:::

Cette architecture permet à opendash de prendre en charge plusieurs clients et vous permet d'interagir avec opendash par programme.

Vous pouvez exécuter `opendash serve` pour démarrer un serveur autonome. Si vous avez le
opendash TUI en cours d'exécution, `opendash serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenDash [IDE](/docs/ide).
...
Le serveur opendash expose les API suivantes.
