---
title: CLI
description: Options et commandes de la CLI OpenDash.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI OpenDash démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
opendash
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec OpenDash par programme.
...
Démarrez l’interface utilisateur du terminal OpenDash.
...
La CLI OpenDash dispose également des commandes suivantes.

---

### agent

Gérer les agents pour OpenDash.
...
Attachez un terminal à un serveur backend OpenDash déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
opendash attach [url]
```

Cela permet d'utiliser le TUI avec un backend OpenDash distant. Par exemple :
...
OpenDash est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `opendash auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/opendash/auth.json`.

```bash
opendash auth login
```

Lorsque OpenDash démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans OpenDash.
...
Exécutez opendash en mode non interactif en transmettant directement un prompt.
...
Vous pouvez également vous connecter à une instance `opendash serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
| `--attach`   |       | Connectez-vous à un serveur opendash en cours d'exécution (par exemple, http://localhost:4096) |
...
Démarrez un serveur OpenDash sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
opendash serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenDash sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opendash`).
...
Liste toutes les sessions OpenDash.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions OpenDash.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage OpenDash.

```bash
opendash import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage OpenDash.
...
Démarrez un serveur OpenDash sans interface graphique avec une interface Web.

```bash
opendash web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenDash via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opendash`).
...
Désinstallez OpenDash et supprimez tous les fichiers associés.
...
Met à jour opendash vers la dernière version ou une version spécifique.
...
La CLI opendash prend les flags globaux suivants.
...
OpenDash peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opendash`) |
