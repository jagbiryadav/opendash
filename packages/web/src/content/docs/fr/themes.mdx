...
Avec OpenDash, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenDash utilise notre propre thème `opendash`.
...
OpenDash est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenDash corresponde à l'apparence de leur terminal
...
OpenDash prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/opendash/themes/*.json` ou `$XDG_CONFIG_HOME/opendash/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.opendash/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.opendash/themes/*.json`
