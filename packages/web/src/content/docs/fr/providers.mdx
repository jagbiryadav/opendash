---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenDash.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenDash utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenDash.
...
en `~/.local/share/opendash/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenDash
configuration.
...
OpenDash Zen est une liste de modèles fournis par l'équipe OpenDash qui ont été
testé et vérifié pour fonctionner correctement avec OpenDash. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenDash Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez opendash et dirigez-vous vers [opendashai.netlify.app/auth](https://opendashai.netlify.app/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenDash et son utilisation est totalement facultative.

---

## OpenDash Go

OpenDash Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe OpenDash qui ont été
testé et vérifié pour fonctionner correctement avec OpenDash.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenDash Go` et rendez-vous sur [opendashai.netlify.app/auth](https://opendashai.netlify.app/zen).
...
Pour utiliser Amazon Bedrock avec OpenDash :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de opendash :
...
Pour une configuration spécifique au projet ou persistante, utilisez `opendash.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenDash n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que opendash fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration opendash.
...
pour stocker le jeton dans le stockage d'authentification opendash.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
OpenDash utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenDash
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `opendash.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `opendash.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec opendash :
...
Pour utiliser Google Vertex AI avec OpenDash :
...
Définissez-les lors de l'exécution de opendash.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via opendash, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opendash-helicone-session](https://github.com/H2Shami/opendash-helicone-session) pour enregistrer automatiquement chaque conversation OpenDash en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenDash répertoriée comme une session distincte.
...
Vous pouvez configurer opendash pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer opendash pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer opendash pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenDash. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opendash) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec OpenDash :
...
3. Copiez la clé API à utiliser dans OpenDash.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenDash, vous devez extraire les informations du modèle localement :
...
OpenDash Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenDash. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenDash Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenDash Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenDash API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration opendash.
...
5. Vous pouvez également les personnaliser via votre configuration opendash. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration opendash. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec opendash. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `opendash.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à OpenDash de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `opendash auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration opendash et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration opendash.
