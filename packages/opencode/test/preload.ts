...
const dir = path.join(os.tmpdir(), "opendash-test-data-" + process.pid)
...
const fixtureRoot = path.join(process.cwd(), ".opendash-test-fixtures-" + process.pid)
await fs.mkdir(fixtureRoot, { recursive: true })
process.env["OPENDASH_TEST_TMPDIR_ROOT"] = fixtureRoot
afterAll(async () => {
...
process.env["OPENDASH_MODELS_PATH"] = path.join(import.meta.dir, "tool", "fixtures", "models-api.json")
...
process.env["OPENDASH_TEST_MANAGED_CONFIG_DIR"] = testManagedConfigDir
process.env["OPENDASH_DISABLE_DEFAULT_PLUGINS"] = "true"

// Write the cache version file to prevent global/index.ts from clearing the cache
const cacheDir = path.join(dir, "cache", "opendash")
...
delete process.env["OPENDASH_SERVER_PASSWORD"]
delete process.env["OPENDASH_SERVER_USERNAME"]
delete process.env["OPENDASH_HOME"]

// Use in-memory sqlite
process.env["OPENDASH_DB"] = ":memory:"
