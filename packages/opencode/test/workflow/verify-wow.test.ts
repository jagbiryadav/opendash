...
// the live router + a real key in ~/.config/opendash/opendash.json). Run with:
...
function realConfig() {
  const home = path.join(os.homedir(), ".config", "opendash", "opendash.json")
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .opendash/workflows.
            const wfDir = path.join(dir, ".opendash", "workflows")
