...
// inherit a parent .git. We temporarily set Flag.OPENDASH_SERVER_PASSWORD to
// bypass the middleware cwd containment check and include auth headers.
const TEST_PASSWORD = "init-git-test"
const authHeader = `Basic ${Buffer.from(`opendash:${TEST_PASSWORD}`).toString("base64")}`

describe("project.initGit endpoint", () => {
  test("initializes git and reloads immediately", async () => {
    const prevFlag = (Flag as any).OPENDASH_SERVER_PASSWORD
    ;(Flag as any).OPENDASH_SERVER_PASSWORD = TEST_PASSWORD
    try {
      ...
      try {
        ...
        expect(await Filesystem.exists(path.join(tmp.path, ".git", "opendash"))).toBe(false)
      ...
      ;(Flag as any).OPENDASH_SERVER_PASSWORD = prevFlag
    }
  })
