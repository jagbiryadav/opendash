...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under opendash.", () => {
    expect(paramToAttributeKey("name")).toBe("opendash.name")
    expect(paramToAttributeKey("slug")).toBe("opendash.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["opendash.sessionID"]).toBeUndefined()
    expect(attrs["opendash.messageID"]).toBeUndefined()
    expect(attrs["opendash.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under opendash. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["opendash.name"]).toBe("exa")
