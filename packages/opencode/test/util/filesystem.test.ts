...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "opendash.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "opendash.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "opendash.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "opendash.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["opendash.json", "opendash.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "opendash.json"),
        path.join(tmp.path, "opendash.jsonc"),
        path.join(project, "opendash.json"),
        path.join(project, "opendash.jsonc"),
      ])
    })
  })
