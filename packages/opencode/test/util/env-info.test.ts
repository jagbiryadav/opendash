...
describe("util.env-info", () => {
  test("returns host, user, runtime, and opendash metadata", async () => {
    ...
    expect(info.opendash.version).toBeTruthy()
    expect(info.opendash.channel).toBeTruthy()
    expect(info.opendash.installation_id).toMatch(
