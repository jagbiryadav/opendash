...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OPENDASH_TEST ?? "")'), {
      env: {
        OPENDASH_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OPENDASH_TEST_SHELL"], {
      shell: true,
      env: {
        OPENDASH_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OPENDASH_TEST_SHELL=ok")
  })
