...
// patterns like `.opendash/` in root .gitignore affect test results.
...
describe("file.ripgrep", () => {
    ...
    withTmpdirOutsideGit(async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".opendash", "thing.json"), "{}")
        },
      })
      ...
      expect(files.includes(path.join(".opendash", "thing.json"))).toBe(true)
    }))
  ...
  test("hidden false excludes hidden", async () => {
    ...
    expect(files.includes("visible.txt")).toBe(true)
    expect(files.includes(path.join(".opendash", "thing.json"))).toBe(false)
  })
