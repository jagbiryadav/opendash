...
import { EffectFlock } from "@opendash-ai/shared/util/effect-flock"
...
const managedConfigDir = process.env.OPENDASH_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "opendash.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "opendash.json") {
...
test("loads Claude Code MCP servers from home and project config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(config.mcp_origins?.context7?.type).toBe("opendash")
...
test("ignores legacy tui keys in opendash config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opendashai.netlify.app/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "opendash.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["OPENDASH_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opendash: { options: { apiKey: "{env:OPENDASH_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["opendash"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["OPENDASH_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENDASH_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "opendash.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .opendash directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendashDir = path.join(dir, ".opendash")
      await fs.mkdir(opendashDir, { recursive: true })
      const agentDir = path.join(opendashDir, "agent")
...
test("loads agents from .opendash/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendashDir = path.join(dir, ".opendash")
      await fs.mkdir(opendashDir, { recursive: true })

      const agentsDir = path.join(opendashDir, "agents")
...
test("loads commands from .opendash/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendashDir = path.join(dir, ".opendash")
      await fs.mkdir(opendashDir, { recursive: true })

      const commandDir = path.join(opendashDir, "command")
...
test("loads commands from .opendash/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendashDir = path.join(dir, ".opendash")
      await fs.mkdir(opendashDir, { recursive: true })

      const commandsDir = path.join(opendashDir, "commands")
...
test("opendash command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".opendash", "command", "dup.md"),
        `---
description: opendash version
---
from opendash`,
      )
    },
...
test("does not try to install dependencies in read-only OPENDASH_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENDASH_CONFIG_DIR
  process.env.OPENDASH_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENDASH_CONFIG_DIR
    else process.env.OPENDASH_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENDASH_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "opendash.json"),
        JSON.stringify({ $schema: "https://opendashai.netlify.app/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .opendash config
      const projectDir = path.join(dir, "project")
      const opendashDir = path.join(projectDir, ".opendash")
      await fs.mkdir(opendashDir, { recursive: true })
      ...
      // Local .opendash config with different plugins
      await Filesystem.write(
        path.join(opendashDir, "opendash.json"),
        JSON.stringify({
          $schema: "https://opendashai.netlify.app/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opendash config with some overlapping plugins
      await Filesystem.write(
        path.join(opendashDir, "opendash.json"),
        JSON.stringify({
          $schema: "https://opendashai.netlify.app/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".opendash")
...
// Note: preload.ts sets OPENDASH_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .opendash config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opendash directory config enables it
      const opendashDir = path.join(dir, ".opendash")
      await fs.mkdir(opendashDir, { recursive: true })
      await Filesystem.write(
        path.join(opendashDir, "opendash.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/opendash")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/opendash")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "opendash.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-opendash@2.4.3", file)).toBe("oh-my-opendash@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opendash@2.4.3", "file:///project/.opendash/plugin/oh-my-opendash.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.opendash/plugin/demo.ts", "file:///project/.opendash/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.opendash/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const opendashDir = path.join(projectDir, ".opendash")
        const pluginDir = path.join(opendashDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENDASH_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENDASH_DISABLE_PROJECT_CONFIG"]
    process.env["OPENDASH_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENDASH_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENDASH_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .opendash/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENDASH_DISABLE_PROJECT_CONFIG"]
    process.env["OPENDASH_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .opendash directory with a command
          const opendashDir = path.join(dir, ".opendash", "command")
          await fs.mkdir(opendashDir, { recursive: true })
          await Filesystem.write(path.join(opendashDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .opendash should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENDASH_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENDASH_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENDASH_CONFIG_DIR"]
      process.env["OPENDASH_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENDASH_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENDASH_CONFIG_DIR"]
      } else {
        process.env["OPENDASH_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENDASH_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENDASH_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENDASH_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENDASH_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENDASH_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from OPENDASH_CONFIG_DIR, not project
...
describe("OPENDASH_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENDASH_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENDASH_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENDASH_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opendashai.netlify.app/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENDASH_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENDASH_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENDASH_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENDASH_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "OpenDash Managed",
          PayloadIdentifier: "ai.opendash.managed.test",
          PayloadType: "ai.opendash.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://opendashai.netlify.app/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://opendashai.netlify.app/config.json")
})
