...
async function isolateZshDotfiles() {
  ...
  const zdotdir = path.join(os.tmpdir(), `opendash-zdotdir-${Math.random().toString(36).slice(2)}`)
  ...
  if (process.platform === "win32") {
    ...
    for (const item of ps) {
      test(
        ...
        withShell(item, async () => {
          const key = "OPENDASH_TEST_MISSING"
            ...
            fn: async () => {
              ...
              await expect(
                Effect.runPromise(
                  bash.execute(
                    {
                      command: "cat /tmp/opendash-does-not-exist",
