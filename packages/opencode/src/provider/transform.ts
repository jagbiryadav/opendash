...
export const OUTPUT_TOKEN_MAX = Flag.OPENDASH_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
// See docs/cache-policy.md and upstream opendash#26786.
...
function limitImages(msgs: ModelMessage[]): ModelMessage[] {
  const maxImages = Flag.OPENDASH_MAX_PROMPT_IMAGES
  const maxSize = Flag.OPENDASH_MAX_PROMPT_IMAGE_SIZE
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "opendash" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("opendash")) {
      result["promptCacheKey"] = input.sessionID
