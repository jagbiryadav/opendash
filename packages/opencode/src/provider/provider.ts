...
import { Npm } from "../npm"
import { Hash } from "@opendash-ai/shared/util/hash"
...
// per-provider via opendash.json's `chunkTimeout` config for tighter or looser
// bounds.
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "opencode-gitlab-auth": () => import("opencode-gitlab-auth").then((m) => m.createGitLab),
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    opendash: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["opendash"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from opendash.json provider config
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      } = yield* Effect.promise(() => import("opencode-gitlab-auth"))
      ...
      const aiGatewayHeaders = {
        "User-Agent": `opendash/${InstallationVersion} opencode-gitlab-auth/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `opendash/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `opendash auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `opendash/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "opendash",
          },
        },
      }),
    kilo: () =>
...
export class Service extends Context.Service<Service, Interface>()("@opendash/Provider") {}
...
/**
 * Generic model discovery for OpenAI-compatible /v1/models endpoints.
 * Works with any provider that exposes a standard models listing API.
 */
export async function discoverModelsFromAPI(
  providerID: string,
  baseURL: string,
  apiKey: string,
  ...
  try {
    ...
    log.info("discovering models from provider API", { providerID, url })

    const response = await fetch(url, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "User-Agent": `opendash/${InstallationVersion}`,
      },
      signal: AbortSignal.timeout(15000),
    })

    if (!response.ok) {
      log.warn("model discovery API returned non-OK", { providerID, status: response.status })
      return discovered
    }
    ...
    for (const model of models) {
      ...
      // Skip models that already exist in models.dev
      ...
      discovered[model.id] = {
        ...
        options: {},
        cost: { input: 0, output: 0, cache: { read: 0, write: 0 } },
        limit: { context: DEFAULT_CONTEXT_WINDOW, output: 8192 },
        capabilities: {
          temperature: false,
          reasoning: false,
          attachment: false,
    ...
    log.warn("model discovery failed", { providerID, error: e instanceof Error ? e.message : String(e) })
  }
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
                ...
                context: (() => {
                  ...
                  if (!warnedContextDefaults.has(key)) {
                    ...
                    log.warn("limit.context not configured and not found in models.dev", {
                      ...
                      fix: `Set limit.context explicitly in opendash.json under provider.${providerID}.models.${modelID}`,
                    })
                  }
        ...
        if (!Flag.OPENDASH_DISABLE_PROVIDER_ENV) {
        ...
        // Generic model discovery for providers with API keys
        // Queries /v1/models for any provider that has auth but might not be in models.dev
        for (const [providerID, provider] of Object.entries(providers)) {
          if (!isProviderAllowed(ProviderID.make(providerID))) continue
          if (Object.keys(provider.models).length > 0) continue // Already has models
          if (!provider.key) continue // No API key
          ...
          yield* Effect.promise(async () => {
            try {
              ...
              log.warn("generic model discovery error", { providerID, error: e })
            }
          })
        }
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            if (model.status === "alpha" && !Flag.OPENDASH_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
