...
import { createOpencodeClient } from "@opendash-ai/sdk"
...
export class Service extends Context.Service<Service, Interface>()("@opendash/Plugin") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createOpencodeClient({
          ...
          headers: Flag.OPENDASH_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.OPENDASH_SERVER_USERNAME ?? "opendash"}:${Flag.OPENDASH_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        const plugins = Flag.OPENDASH_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.OPENDASH_PURE && cfg.plugin_origins?.length) {
