...
export const DEPRECATED_PLUGIN_PACKAGES = ["opendash-openai-codex-auth", "opendash-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, opendashVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(opendashVersion) || semver.major(opendashVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.opendash
  if (typeof range !== "string") return
  if (!semver.satisfies(opendashVersion, range)) {
    throw new Error(`Plugin requires opendash ${range} but running ${opendashVersion}`)
  }
}
