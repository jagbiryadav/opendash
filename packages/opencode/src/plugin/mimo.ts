import type { Hooks, PluginInput, ProviderHookContext } from "@opendash-ai/plugin"
import type { Model as ModelV2 } from "@opendash-ai/sdk/v2"
...
const PLATFORM_URL = process.env.MIMO_PLATFORM_URL || "https://platform.opendashai.netlify.app"
...
export async function MimoAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      ...
      // api: https://api.opendashai.netlify.app/v1) — hardcoding "MiMo" here collided with
      // the free "mimo" provider's display name and confused users.
      input.provider.xiaomi ??= {}
      // "opendash" stays enabled with a custom loader that strips the free/public
      // tier and hides paid models until the user authenticates.
    },
    provider: {
      id: "opendash",
      async models(provider, ctx) {
        // Fetch models directly from OpenDash Zen API — the single source of truth
        const apiKey = ctx.auth?.type === "api" ? ctx.auth.key : undefined
        if (!apiKey) return provider.models

        try {
          const result = await fetch("https://opendash.ai/zen/v1/models", {
            headers: { Authorization: `Bearer ${apiKey}` },
            signal: AbortSignal.timeout(10000),
          })
          if (!result.ok) return provider.models

          const data = await result.json() as { data?: Array<{ id: string; owned_by?: string }> }
          ...
          for (const m of data.data) {
            models[m.id] = {
              ...
              api: {
                id: m.id,
                url: "https://opendash.ai/zen/v1",
                npm: "@ai-sdk/openai-compatible",
              },
              capabilities: {
                temperature: true,
                reasoning: m.id.includes("thinking") || m.id.includes("spark"),
                attachment: false,
                toolcall: true,
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers["X-Mimo-Source"] = "opendash-cli"
    },
  }
}
