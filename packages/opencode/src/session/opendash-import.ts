...
export async function run(opts?: { force?: boolean; dbPath?: string }): Promise<ImportStats> {
      ...
      try {
        ...
        let existing = Database.use((db) =>
          db
            ...
            .where(and(eq(ExternalImportTable.source, "opendash"), eq(ExternalImportTable.source_key, sourceKey)))
            .get(),
        )
        // Skip only when the source session is unchanged since last import. We
        // store time_updated as source_mtime, so a re-edited opendash session
        ...
        // If this import previously created a opendash session that the user has
        ...
        // session backward past opendash-native activity (cc/codex parity).
        ...
        if (existing) {
          ...
          if (!mimoSess) {
            Database.use((db) =>
              db.delete(ExternalImportTable).where(and(eq(ExternalImportTable.source, "opendash"), eq(ExternalImportTable.source_key, sourceKey))).run(),
            )
            existing = undefined
          } else {
  ...
  if (stats.imported + stats.resynced > 0 || stats.errors.length > 0)
    log.info("opendash import", { ...stats, errors: stats.errors.length })
  return stats
}

export * as OpencodeImport from "./opendash-import"
