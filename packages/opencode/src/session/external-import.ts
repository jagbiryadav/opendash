...
import { Glob } from "@opendash-ai/shared/util/glob"
...
import { OpencodeImport, DEFAULT_DB_PATH } from "./opendash-import"

const log = Log.create({ service: "external-import" })

export type Source = "cc" | "codex" | "opendash"
export const ALL_SOURCES: readonly Source[] = ["cc", "codex", "opendash"] as const
...
export async function scan(): Promise<ScanResult> {
  ...
  const result: ScanResult = {
    ...
    opendash: { available: false, sessions: 0, imported: 0 },
  }
  ...
  if (existsSync(DEFAULT_DB_PATH)) {
    try {
      ...
      try {
        ...
        result.opendash = { available: true, sessions: row?.n ?? 0, imported: importedBySource("opendash") }
      } finally {
        db.close()
      }
    } catch (e) {
      log.warn("opendash scan failed", { error: String(e) })
    }
  }
...
export async function runAll(opts?: {
  ...
  for (const source of ALL_SOURCES) {
    ...
    try {
      if (source === "cc") {
        ...
        result.opendash = await OpencodeImport.run({ force: opts?.force })
      }
    } catch (e) {
  ...
  const totals = {
    scanned: result.cc.scanned + result.codex.scanned + result.opendash.scanned,
    imported: result.cc.imported + result.codex.imported + result.opendash.imported,
    resynced: result.cc.resynced + result.codex.resynced + result.opendash.resynced,
    errors: result.cc.errors.length + result.codex.errors.length + result.opendash.errors.length,
  }
