...
import { ensureProcessMetadata } from "./util/opendash-process"
...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    process.env.AGENT = "1"
    process.env.OPENDASH_PID = String(process.pid)

    Log.Default.info("opendash", {
    ...
    const marker = path.join(Global.Path.data, "opendash.db")
    ...
    if (!process.env.OPENDASH_DISABLE_CLAUDE_IMPORT && !process.env.OPENDASH_CLAUDE_IMPORTED) {
      process.env.OPENDASH_CLAUDE_IMPORTED = "1"
      try {
