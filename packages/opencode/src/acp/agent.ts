...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@opendash-ai/sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `opendash auth login` in the terminal",
      name: "Login with opendash",
      id: "opendash-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // OpenDash stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const opendashProvider = providers.find((p) => p.id === "opendash")
  if (opendashProvider) {
    if (opendashProvider.models["big-pickle"]) {
      return { providerID: ProviderID.opendash, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(opendashProvider.models))
    if (best) {
      return {
