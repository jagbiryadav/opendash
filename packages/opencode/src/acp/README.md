...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for opendash.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal opendash sessions
...
opendash acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 opendash acp
```
...
- **Terminal Integration**: Full terminal support via opendash's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | opendash acp
```
...
ACP sessions map cleanly to opendash's internal session model:
