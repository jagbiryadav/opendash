...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@opendash-ai/plugin"
import z from "zod"
...
export class Service extends Context.Service<Service, Interface>()("@opendash/ToolRegistry") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.OPENDASH_CLIENT) || Flag.OPENDASH_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.patch,
            tool.changedir,
            ...(Flag.OPENDASH_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            tool.planexit,
            tool.planenter,
            tool.memory,
            tool.history,
            tool.task,
            ...(Flag.OPENDASH_EXPERIMENTAL_WORKFLOW_TOOL ? [tool.workflow] : []),
          ],
          actor: tool.actor,
          read: tool.read,
        }
      ...
      let filtered = (yield* all()).filter((tool) => {
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          if (tool.id === WebSearchTool.id) {
            return (
              input.providerID === ProviderID.opendash ||
              input.providerID === "xiaomi" ||
              Flag.OPENDASH_ENABLE_EXA
            )
          }
          return input.providerID === ProviderID.opendash || Flag.OPENDASH_ENABLE_EXA
        }
