...
import type { GlobalEvent } from "@opendash-ai/sdk/v2"
import type { EventSource } from "./context/sdk"
import { win32DisableProcessedInput, win32InstallCtrlCGuard } from "./win32"
import { writeHeapSnapshot } from "v8"
import { TuiConfig } from "./config/tui"
import { OPENDASH_PROCESS_ROLE, OPENDASH_RUN_ID, ensureRunID, sanitizedProcessEnv } from "@/util/opendash-process"
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start opendash tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start opendash in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [OPENDASH_PROCESS_ROLE]: "worker",
        [OPENDASH_RUN_ID]: ensureRunID(),
      })
