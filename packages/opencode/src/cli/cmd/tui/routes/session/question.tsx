...
import type { QuestionAnswer, QuestionRequest } from "@opendash-ai/sdk/v2"
...
export function QuestionPrompt(props: { request: QuestionRequest }) {
  ...
  const translateQuestion = (q: QuestionRequest["questions"][number]) => {
    if (!q.key) return q
    const prefix = `tui.question.${q.key}`
    const translated = t(`${prefix}.question`, q.params ?? {})
    if (!translated) return q
    return {
      ...q,
      question: translated,
      header: t(`${prefix}.header`) || q.header,
      options: q.options.map((opt, i) => ({
        label: t(`${prefix}.option.${i}.label`) || opt.label,
        description: t(`${prefix}.option.${i}.description`) || opt.description,
      })),
    }
  }

  const questions = createMemo(() => props.request.questions.map(translateQuestion))
  ...
  function selectOption() {
    ...
    if (!opt) return
    const originalLabel = props.request.questions[store.tab]?.options[store.selected]?.label ?? opt.label
    if (multi()) {
      toggle(originalLabel)
      return
    }
    pick(originalLabel)
  }
            ...
            <box>
              <For each={options()}>
                {(opt, i) => {
                  ...
                  const originalLabel = () => props.request.questions[store.tab]?.options[i()]?.label ?? opt.label
                  const picked = () => store.answers[store.tab]?.includes(originalLabel()) ?? false
                  return (
                    <box
