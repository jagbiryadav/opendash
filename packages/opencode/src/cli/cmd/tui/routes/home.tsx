...
export function Home() {
  ...
  const centeredThinRows = createMemo(() => {
    const GAP = 1
    const full = logoThin.left.map((line, i) => line + " ".repeat(GAP) + logoThin.right[i])
    const maxLen = Math.max(...full.map((r) => r.length))
    const termWidth = dimensions().width
    const pad = Math.max(0, Math.floor((termWidth - 16 - maxLen) / 2))
    return full.map((row) => " ".repeat(pad) + row)
  })
        ...
        <box flexShrink={0}>
          ...
          >
            <box flexDirection="column" flexShrink={0}>
              {centeredThinRows().map((row) => (
                <text selectable={false}>{row}</text>
              ))}
            </box>
          </Show>
        </box>
