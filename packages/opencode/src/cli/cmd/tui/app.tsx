...
function rendererConfig(_config: TuiConfig.Info, plainTerminal: boolean): CliRendererConfig {
  const mouseEnabled = !plainTerminal && !Flag.OPENDASH_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  useKeyboard((evt) => {
    if (!Flag.OPENDASH_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.OPENDASH_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("OpenDash")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`OA | ${title}`)
      return
    }
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "workflow.list",
      category: "session",
      enabled: Flag.OPENDASH_EXPERIMENTAL_WORKFLOW_TOOL,
      slash: {
        name: "workflows",
      },
      onSelect: () => {
    ...
    ...(sync.data.console_state.switchableOrgCount > 1
      ...
      : []),
    {
      title: t("tui.command.opendash.status.title"),
      keybind: "status_view",
      value: "opendash.status",
      slash: {
        name: "status",
      },
      onSelect: () => {
    ...
    },
    {
      ...
      onSelect: () => {
        open("https://opendash-ai.vercel.app/docs").catch(() => {})
        dialog.clear()
      },
      category: "system",
    },
  ...
  return (
    <box
      ...
      onMouseDown={(evt) => {
        ...
        if (
          Flag.OPENDASH_EXPERIMENTAL_DISABLE_COPY_ON_SELECT &&
    ...
    >
      <Show when={Flag.OPENDASH_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <Switch>
