...
export function resolveCredentials(
  ...
  // If voice config has explicit API key, use it directly (no need for provider to be connected)
  if (overrides?.apiKey) {
    const baseUrl = overrides?.baseUrl
      || getDefaultBaseUrl(config.providerID)
      || (providers.find((p) => p.id === config.providerID)?.options?.baseURL as string)
      || Object.values(providers.find((p) => p.id === config.providerID)?.models ?? {})[0]?.api?.url
    if (!baseUrl) return { error: "no_url", providerID: config.providerID, model: config.model }
    return { apiKey: overrides.apiKey, baseUrl }
  }
...
function getDefaultBaseUrl(providerID: string): string | undefined {
  const defaults: Record<string, string> = {
    xiaomi: "https://opendash.ai/zen/v1",
    opendash: "https://opendash.ai/zen/v1",
...
export function resolveVoiceConfig(voiceConfig?: {
  ...
  return {
    ...
    asrApiKey: voiceConfig?.asr_api_key,
    asrBaseUrl: voiceConfig?.asr_base_url,
  }
}
...
// Known STT models for voice provider selection
...
// Dispatch transcription based on provider
export async function transcribeAudio(opts: {
  ...
  const providerID = opts.providerID || "xiaomi"
  if (providerID === "sarvam") return transcribeSarvam(opts)
  if (providerID === "xiaomi" || providerID === "mimo") return transcribeXiaomi(opts)
  return transcribeGeneric(opts)
}

// Xiaomi ASR uses a proprietary data-URL audio format and asr_options field
async function transcribeXiaomi(opts: {
  audio: Int16Array
  apiKey: string
  baseUrl: string
  model?: string
  ...
  log.debug("transcribe request", { provider: "xiaomi", model, samples })
  ...
  if (!res || !res.ok) {
    ...
    log.warn("transcribe failed", { provider: "xiaomi", model, status: res?.status, body })
    return null
  }
  try {
    const data = (await res.json()) as { choices?: Array<{ message?: { content?: string } }> }
    const text = data.choices?.[0]?.message?.content?.trim() || null
    log.debug("transcribe result", { provider: "xiaomi", model, length: text?.length ?? 0 })
    return text
  } catch {
    return null
  }
...
// OpenAI-compatible STT: works for OpenAI Whisper, Groq, Deepgram, and any compatible endpoint
async function transcribeGeneric(opts: {
  audio: Int16Array
  apiKey: string
  baseUrl: string
  model?: string
  ...
  const samples = opts.audio.length
  log.debug("transcribe request", { provider: "generic", model, samples })
  const wavBuffer = encodeWav(opts.audio)
  ...
  if (!res || !res.ok) {
    const body = await res?.text().catch(() => "")
    log.warn("transcribe failed", { provider: "generic", model, status: res?.status, body })
    return null
  }
  try {
    const data = (await res.json()) as { text?: string }
    const text = data.text?.trim() || null
    log.debug("transcribe result", { provider: "generic", model, length: text?.length ?? 0 })
    return text
  } catch {
    return null
  }
...
// Sarvam.ai STT: uses x-api-key header and their own endpoint format
async function transcribeSarvam(opts: {
  audio: Int16Array
  apiKey: string
  baseUrl: string
  model?: string
}): Promise<string | null> {
  const model = opts.model || "saaras:v2"
  const samples = opts.audio.length
  log.debug("transcribe request", { provider: "sarvam", model, samples })
  const wavBuffer = encodeWav(opts.audio)
  const baseUrl = opts.baseUrl || "https://api.sarvam.ai"
  ...
  if (!res || !res.ok) {
    const body = await res?.text().catch(() => "")
    log.warn("transcribe failed", { provider: "sarvam", model, status: res?.status, body })
    return null
  }
  try {
    const data = (await res.json()) as { transcript?: string; text?: string }
    const text = (data.transcript || data.text)?.trim() || null
    log.debug("transcribe result", { provider: "sarvam", model, length: text?.length ?? 0 })
    return text
  } catch {
    return null
  }
...
const VOICE_CONTROL_SYSTEM_PROMPT = `你是 OpenDash（AI 编程助手）的语音输入助手。用户通过语音向输入框口述消息，这些消息将发送给 Code Agent 执行编程任务。用户可能使用中文或英文。
