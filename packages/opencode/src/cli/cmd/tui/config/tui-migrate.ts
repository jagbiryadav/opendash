...
const TUI_SCHEMA_URL = "https://opendash-ai.vercel.app/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from opendash.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const opendash = await opendashFiles(input)
  for (const file of opendash) {
...
async function opendashFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "opendash"),
    ...(await Filesystem.findUp(["opendash.json", "opendash.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opendash"))
  }
  if (Flag.OPENDASH_CONFIG) files.push(Flag.OPENDASH_CONFIG)
