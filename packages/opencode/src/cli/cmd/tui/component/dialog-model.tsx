...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "opendash" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
        ...
        return [
          {
            ...
            category,
            disabled: provider.id === "opendash" && model.id.includes("-nano"),
            footer: model.cost?.input === 0 && provider.id === "opendash" ? "Free" : undefined,
            onSelect: () => {
      ...
      flatMap((provider) => {
        const models = pipe(
          ...
          map(([model, info]) => ({
            ...
            disabled: provider.id === "opendash" && model.includes("-nano"),
            footer: info.cost?.input === 0 && provider.id === "opendash" ? "Free" : undefined,
            onSelect() {
