...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; sweep?: boolean } = {}) {
  ...
  const OPEN_ANALYST_BLUE = RGBA.fromInts(3, 76, 255)
  const OPEN_ANALYST_GRAY = RGBA.fromInts(107, 115, 148)
          ...
          if (labelRow) {
            return (
              <box flexDirection="row" gap={1}>
                <text fg={OPEN_ANALYST_GRAY} selectable={false}>{line}</text>
                <text fg={OPEN_ANALYST_GRAY} selectable={false}>{ctx.shape.right[index()]}</text>
              </box>
            )
          }
          return (
            <box flexDirection="row" gap={1}>
              <box flexDirection="row">
                {renderLine(line, index(), props.ink ?? OPEN_ANALYST_BLUE, true, 0, frame(), dusk(), idleState())}
              </box>
              <box flexDirection="row">
                {renderLine(
                  ctx.shape.right[index()],
                  index(),
                  props.ink ?? OPEN_ANALYST_GRAY,
                  true,
                  ctx.LEFT + GAP,
                  frame(),
                  dusk(),
