...
import opendash from "./theme/opendash.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@opendash-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "opendash")
        draft.active = typeof active === "string" ? active : "opendash"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.opendash, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const brandBlue = RGBA.fromInts(3, 76, 255)
  const brandTeal = RGBA.fromInts(0, 212, 170)

  return {
    theme: {
      // Primary colors using brand blue
      primary: brandBlue,
      secondary: RGBA.fromInts(91, 141, 239),
      accent: brandTeal,
