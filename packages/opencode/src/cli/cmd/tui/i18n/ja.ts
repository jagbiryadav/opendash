...
export const dict = {
  ...
  "tui.home.agreement.prefix": "OpenDash をご利用いただくことで、",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}opendash.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/opendash/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "OpenDash は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.opendash/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.opendash/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "OpenDash は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "OpenDash はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.opendash/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.opendash/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて OpenDash が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}oa run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}opendash --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}oa run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}oa serve{/highlight} を実行するとヘッドレスで OpenDash API にアクセスできます",
  "tui.tips.attach_server": "{highlight}oa run --attach{/highlight} で起動中のサーバに接続します",
  "tui.tips.upgrade": "{highlight}oa upgrade{/highlight} で最新バージョンに更新します",
  "tui.tips.auth_list": "{highlight}opendash auth list{/highlight} で設定済みプロバイダを確認します",
  "tui.tips.agent_create": "{highlight}opendash agent create{/highlight} でガイド付きエージェント作成を行います",
  "tui.tips.github_trigger": "GitHub の issue/PR で {highlight}/opendash{/highlight} を入力すると AI を起動します",
  "tui.tips.github_install": "{highlight}opendash github install{/highlight} で GitHub workflow を設定します",
  ...
  "tui.tips.theme_files": "{highlight}.opendash/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}opendash debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.tips.docker":
    "{highlight}docker run -it --rm ghcr.io/opendash/opendash{/highlight} でコンテナ版を実行できます",
  ...
  "tui.command.opendash.status.title": "ステータスを表示",
  ...
  "tui.model.mimo_auto.name": "OpenDash（MiMo-V2.5 期間限定無料）",
  ...
  "tui.toast.update_available.success": "OpenDash v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "tui.dialog.login.mimo_free.success": "OpenDash (free) の準備完了 — デフォルトモデルを mimo/mimo-auto に設定しました",
  "tui.dialog.login.mimo_free.unavailable": "OpenDash (free) プロバイダーが読み込まれていません",
  ...
  "cli.providers.mimo_free.verifying": "OpenDash (free) チャネルを検証中...",
  "cli.providers.mimo_free.ready": "OpenDash (free) チャネル準備完了",
  "cli.providers.mimo_free.failed": "OpenDash (free) の自己診断に失敗",
  "cli.providers.mimo_free.default_set": "デフォルトモデルを mimo/mimo-auto に設定（1M コンテキスト、無料）",
  "cli.providers.mimo_free.usage_hint":
    "ログイン不要 — そのまま opendash を実行できます。有料/上位モデルを利用する場合は MiMo ブラウザログインを選択してください。",
