...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall opendash and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenDash")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @opendash-ai/cli",
      pnpm: "pnpm uninstall -g @opendash-ai/cli",
      bun: "bun remove -g @opendash-ai/cli",
      // TODO(opendash): uncomment when published to these channels
      // brew: "brew uninstall opendash",
      // choco: "choco uninstall opendash",
      // scoop: "scoop uninstall opendash",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenDash!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opendash") || content.includes(".opendash/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opendash/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".opendash/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".opendash"))
    ) {
      continue
    }
