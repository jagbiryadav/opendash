    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: opendash mcp add")
          return
        }
        ...
        for (const [name, serverConfig] of servers) {
          ...
          const typeHint =
            serverConfig.type === "remote"
              ? ConfigMCP.redactString(serverConfig.url)
              : ConfigMCP.redactCommand(serverConfig.command).join(" ")
          prompts.log.info(
            `${statusIcon} ${name} ${UI.Style.TEXT_DIM}${statusText} ${sourceLabel(config, name)}${hint}\n    ${UI.Style.TEXT_DIM}${typeHint}`,
          )
        }
...
export const McpAuthCommand = cmd({
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in opendash.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
  ...
  "mcp": {
    "${serverName}": {
      "type": "remote",
      "url": "${ConfigMCP.redactString(serverConfig.url)}",
      "oauth": {
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        for (const [name, serverConfig] of servers) {
          ...
          const icon = getAuthStatusIcon(authStatus)
          const statusText = getAuthStatusText(authStatus)
          const url = ConfigMCP.redactString(serverConfig.url)
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .opendash/ subdirectory too)
  const candidates = [path.join(baseDir, "opendash.json"), path.join(baseDir, "opendash.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".opendash", "opendash.json"), path.join(baseDir, ".opendash", "opendash.jsonc"))
  }
  ...
  // Default to opendash.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., opendash x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        prompts.log.info(`URL: ${ConfigMCP.redactString(serverConfig.url)}`)
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "opendash-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
