...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run opendash",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for opendash session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found opendash session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["opendash", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting opendash...")
        UI.println()

        const mimoArgs = sessionId ? ["-s", sessionId] : []
        const mimoProcess = Process.spawn(["opendash", ...mimoArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        ...
        if (code !== 0) throw new Error(`opendash exited with code ${code}`)
      },
    })
  },
})
