...
import type { Hooks } from "@opendash-ai/plugin"
...
export const ProvidersLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "opendash auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          ...
          const wellknown = (await fetch(`${url}/.well-known/opendash`).then((x) => x.json())) as {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in opendash.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via opendash.json options (profile, region, endpoint) or\n" +
              "AWS environment variables (AWS_PROFILE, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_WEB_IDENTITY_TOKEN_FILE).",
          )
        }

        if (provider === "opendash") {
          prompts.log.info("Obtain an API key from your selected provider (OpenAI, Anthropic, OpenRouter, Gemini, etc.)")
        }
        ...
        if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
          prompts.log.info(
            "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://opendash-ai.vercel.app/docs/providers/#cloudflare-ai-gateway",
          )
        }
...
export const ProvidersWhoamiCommand = cmd({
  ...
  async handler(_args) {
    ...
    if (!info) {
      prompts.log.error("Not logged in. Run `opendash auth login` to log in.")
      return
    }
