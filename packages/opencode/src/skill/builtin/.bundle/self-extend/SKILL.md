...
You can extend your own capabilities by writing files to `.opendash/`. Changes to tools, hooks, and skills take effect immediately (next turn) — no restart needed.

## Creating Tools

Write to `.opendash/tools/<name>.ts`:
...
Write to `.opendash/hooks/<name>.ts` — export a Hooks object:
...
```ts
// .opendash/tools/bash.ts — overrides built-in bash
import { tool } from "@mimo-ai/plugin"
...
Write to `.opendash/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.opendash/tools/*.ts` | next turn |
| Hooks | `.opendash/hooks/*.ts` | next turn |
| Skills | `.opendash/skills/*/SKILL.md` | next turn |
| TUI | `.opendash/tui/*.tsx` | restart |
