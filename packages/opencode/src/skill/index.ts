...
const EXTERNAL_DIRS = [".claude", ".agents", ".codex", ".opendash"]
const EXTERNAL_SKILL_PATTERN = "skills/**/SKILL.md"
const OPENDASH_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.OPENDASH_DISABLE_BUILTIN_SKILLS) {
  ...
  if (!Flag.OPENDASH_DISABLE_COMPOSE_SKILLS) {
  ...
  if (!Flag.OPENDASH_DISABLE_EXTERNAL_SKILLS) {
    const externalDirs = EXTERNAL_DIRS.filter((dir) => {
      if (dir === ".claude" && Flag.OPENDASH_DISABLE_CLAUDE_CODE_SKILLS) return false
      if (dir === ".codex" && Flag.OPENDASH_DISABLE_CODEX_SKILLS) return false
      if (dir === ".opendash" && Flag.OPENDASH_DISABLE_OPENCODE_SKILLS) return false
      return true
    })
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, OPENDASH_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@opendash/Skill") {}
