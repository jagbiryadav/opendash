...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".opendash-project-id")
  const idFile = nodePath.join(mainGit, "opendash-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .opendash-project-id
  ...
  if (!existing.includes(".opendash-project-id")) {
...
export class Service extends Context.Service<Service, Interface>()("@opendash/Project") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.OPENDASH_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        // turn, which has shipped as a bug for users opening opendash in $HOME.
      ...
      if (Flag.OPENDASH_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
