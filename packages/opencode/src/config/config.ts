...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in opendash config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
const InfoSchema = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for opendash serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://opendash-ai.vercel.app/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://opendash-ai.vercel.app/docs/agents" }),
  ...
  memory: Schema.optional(
    Schema.Struct({
      cc_index: Schema.optional(Schema.Boolean).annotate({
        description:
          "Index Claude Code memory (~/.claude/projects/<slug>/memory) and expose under scope='cc'. Default: false. Note: when enabled, every opendash agent (build/explore/subagents) can search these memories via the builtin `memory` tool — including CC's `type: user` (your role/preferences) and `type: feedback` (your guidance) categories. CC originally writes them for future CC sessions; flipping this on widens the consumer set to opendash agents on the same machine. Leave disabled (default) if you don't want personal context recallable from a prompt-injection-vulnerable agent.",
      }),
    }),
  ),
  ...
  voice: Schema.optional(
    Schema.Struct({
      asr_model: Schema.optional(ConfigModelID).annotate({
        description:
          "Model to use for voice ASR transcription in provider/model format. Defaults to xiaomi/mimo-v2.5-asr. Supports openai/whisper-1, groq/whisper-large-v3, deepgram/nova-2, sarvam/saaras:v2.",
      }),
      ...
      asr_api_key: Schema.optional(Schema.String).annotate({
        description:
          "API key for the voice ASR provider. If not set, falls back to the connected provider's credentials.",
      }),
      asr_base_url: Schema.optional(Schema.String).annotate({
        description:
          "Base URL for the voice ASR provider. If not set, resolved from the provider config.",
      }),
    }),
...
export class Service extends Context.Service<Service, Interface>()("@opendash/Config") {}

function globalConfigFile() {
  const candidates = ["opendash.jsonc", "opendash.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://opendash-ai.vercel.app/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "opendash.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "opendash.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://opendash-ai.vercel.app/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OPENDASH_CONFIG_CONTENT") return "local"
        ...
        const merge = (source: string, next: Info, kind?: ConfigPlugin.Scope) => {
          ...
          mergeMcpOrigins(source, next, "opendash")
        ...
        const mergeClaudeMcp = Effect.fnUntraced(function* (source: string) {
          ...
          for (const [name, server] of Object.entries(data.mcpServers)) {
            ...
            if (existing && result.mcp_origins?.[name]?.type !== "claude") {
              log.info(`skipped Claude Code MCP server "${name}"; native opendash MCP with same name already exists.`)
              continue
            }
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/opendash` })
            const response = yield* Effect.promise(() =>
              fetch(`${url}/.well-known/opendash`, { signal: AbortSignal.timeout(1000) }),
            )
            if (!response.ok) {
            ...
            const remoteConfig = wellknown.config ?? {}
             if (!remoteConfig.$schema) remoteConfig.$schema = "https://opendash-ai.vercel.app/config.json"
            const source = `${url}/.well-known/opendash`
        ...
        if (Flag.OPENDASH_CONFIG) {
          yield* merge(Flag.OPENDASH_CONFIG, yield* loadFile(Flag.OPENDASH_CONFIG))
          log.debug("loaded custom config", { path: Flag.OPENDASH_CONFIG })
        }

        if (!Flag.OPENDASH_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("opendash", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.OPENDASH_CONFIG_DIR) {
          log.debug("loading config from OPENDASH_CONFIG_DIR", { path: Flag.OPENDASH_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .opendash commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".opendash") || dir === Flag.OPENDASH_CONFIG_DIR) {
            for (const file of ["opendash.json", "opendash.jsonc"]) {
          ...
          // Auto-discovered plugins under `.opendash/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OPENDASH_CONFIG_CONTENT) {
          const source = "OPENDASH_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OPENDASH_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from OPENDASH_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OPENDASH_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OPENDASH_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (managed) {
          ...
          result = mergeConfigConcatArrays(result, next)
          mergeMcpOrigins(managed.source, next, "opendash")
        }

        if (!Flag.OPENDASH_DISABLE_CLAUDE_CODE_MCP) {
        ...
        if (Flag.OPENDASH_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENDASH_PERMISSION))
        }
        ...
        if (Flag.OPENDASH_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OPENDASH_DISABLE_PRUNE) {
