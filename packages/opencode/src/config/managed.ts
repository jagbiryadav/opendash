...
const MANAGED_PLIST_DOMAIN = "ai.opendash.managed"

// Keys injected by macOS/MDM into the managed plist that are not OpenDash config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/opendash"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "opendash")
    default:
      return "/etc/opendash"
  }
}
...
export function managedConfigDir() {
  return process.env.OPENDASH_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
