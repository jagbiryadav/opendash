...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    OPENDASH_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    OPENDASH_WORKSPACE_ID: config.id,
    OPENDASH_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.OPENDASH_EXPERIMENTAL_WORKSPACES) return
