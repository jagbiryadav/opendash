...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/opendash`,
      execArgv: [`--user-agent=opendash/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: embeddedFileMap ? { "opendash-web-ui.gen.ts": embeddedFileMap } : {},
    entrypoints: ["./src/index.ts", parserWorker, workerPath, ...(embeddedFileMap ? ["opendash-web-ui.gen.ts"] : [])],
    define: {
      OPENDASH_VERSION: `'${Script.version}'`,
      OPENDASH_MIGRATIONS: JSON.stringify(migrations),
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + workerRelativePath,
      OPENCODE_WORKER_PATH: workerPath,
      OPENDASH_CHANNEL: `'${Script.channel}'`,
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/opendash`
  ...
  await Bun.file(`dist/${name}/README.md`).write(
    `This is the ${item.os}-${item.arch} binary for [@opendash-ai/cli](https://www.npmjs.com/package/@opendash-ai/cli). Install that package directly.\n`,
  )
  await Bun.file(`dist/${name}/package.json`).write(
    JSON.stringify(
      {
        name: `@opendash-ai/${name}`,
        version: Script.version,
        description: "Platform-specific binary for @opendash-ai/cli.",
        license: "MIT",
        ...
        keywords: ["ai", "coding", "agent", "cli", "opendash"],
        os: [item.os],
        cpu: [item.arch],
        bin: {
          opendash: `./bin/opendash${item.os === "win32" ? ".exe" : ""}`,
        },
        files: ["bin"],
      },
      null,
