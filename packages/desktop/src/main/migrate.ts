...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.opendash.desktop.dev",
  beta: "ai.opendash.desktop.beta",
  prod: "ai.opendash.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.opendash.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `opendash.settings.dat` is special: it maps to the `opendash.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"opendash.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // opendash.settings.dat → the electron settings store ("opendash.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "opendash.global.dat").
  const storeName = filename === "opendash.settings.dat" ? "opendash.settings" : filename
