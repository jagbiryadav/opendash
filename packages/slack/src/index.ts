...
import { createOpencode, type ToolPart } from "@opendash-ai/sdk"
...
console.log("🚀 Starting opendash server...")
const opendash = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await opendash.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new opendash session...")
    const { client, server } = opendash
    ...
    console.log("✅ Created opendash session:", createResult.data.id)
  ...
  console.log("📝 Sending to opendash:", message.text)
