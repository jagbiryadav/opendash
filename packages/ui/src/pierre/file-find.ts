...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("opendash-find")
  api.delete("opendash-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("opendash-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("opendash-find", new Highlight(...rest))
    return true
  }
